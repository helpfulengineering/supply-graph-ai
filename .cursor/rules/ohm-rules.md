...
This repository has a unified map (`.repo-map.md`) with two complementary sections. Select the section based on the question type.

### Available Repository Map

**`.repo-map.md`** - Unified repository map containing:

1. **Aider-style section** (first)
...
2. **Sourcegraph-style section** (second)
...
**Use the Aider-style section (first) when the user asks:**
...
**Use the Sourcegraph-style section (second) when the user asks:**
...
**Use BOTH sections when the user asks:**
...
**Skip maps when the user asks:**
- ❌ **"How does [specific function] work?"** → Implementation detail; search directly in code
- ❌ **"Fix this bug in [file]..."** → Specific file issue; inspect the file directly

### Step 2: Execute Repository Map Workflow

**For Aider-style questions (location/structure):**

1. **FIRST**: Read `.repo-map.md` with `ReadFile`, focusing on the Aider-style section
2. **THEN**: Use `rg` or `SemanticSearch` for specific implementation details
3. **FINALLY**: Cross-reference with actual code files

**For Sourcegraph-style questions (relationships/APIs):**

1. **FIRST**: Read `.repo-map.md` with `ReadFile`, focusing on the Sourcegraph-style section and relevant categories (Services, Models, Utilities, etc.)
2. **THEN**: Use `rg` or `SemanticSearch` for specific implementation details
3. **FINALLY**: Cross-reference with actual code files

**For comprehensive questions (use both sections):**

1. **FIRST**: Read the Sourcegraph-style section to understand what exists (component discovery)
2. **SECOND**: Read the Aider-style section to understand where components live (file structure)
3. **THEN**: Use `rg` or `SemanticSearch` for specific implementation details
...
- Helps maintain architectural consistency and reduce duplicate code

### Code Organization Rules

1. **Check the appropriate section(s) first** - Use Aider for location questions, Sourcegraph for component/API questions
...
5. **Respect file structure** - Use Aider to ensure new code goes in the correct location
...
**Example 1: "Where should I add a new service for BOM post-processing?"**
- ✅ Use: **Aider section** of `.repo-map.md` (location question)
- Look for: `src/core/services/` and `src/core/generation/services/` in Aider section to compare existing service locations
- Then check: Sourcegraph "Services" category for concrete patterns in files such as `src/core/services/bom_resolution_service.py` and `src/core/generation/services/repository_mapping_service.py`

**Example 2: "What utility helpers already exist for generation and schemas?"**
- ✅ Use: **Sourcegraph section** of `.repo-map.md` (component discovery question)
- Look for: "Utilities" category entries like `src/core/generation/utils/text_processor.py`, `src/core/utils/schema_generator.py`, and `src/core/utils/logging.py`
- Then check: Aider section for exact file locations under `src/core/generation/utils/` and `src/core/utils/`

**Example 3: "I want to add a new CLI command for batch matching. Where does it go and what existing patterns should I follow?"**
- ✅ Use: **BOTH sections** of `.repo-map.md` (comprehensive question)
- Sourcegraph section: "Entry Points" → `src/cli/main.py` for CLI registration
- Aider section: `src/cli/` directory structure to find where subcommand groups live (e.g., `match.py`, `okh.py`, `utility.py`)
- Sourcegraph section: "Services" and "Utilities" entries related to matching, such as `src/core/services/matching_service.py` and `src/core/matching/rules_service.py`

**Example 4: "What's the public API of the matching service?"**
- ✅ Use: **Sourcegraph section** of `.repo-map.md` (API question)
- Look for: "Services" category → `src/core/services/matching_service.py` and `src/core/matching/rules_service.py` → **Exports** and **Methods**

**Example 5: "Show me the structure of generation utilities"**
- ✅ Use: **Aider section** of `.repo-map.md` (structure question)
- Look for: `src/core/generation/utils/` tree structure
...
**CRITICAL: Prefer a Python 3.12 environment (Conda recommended) for local commands.**

- **Preferred Environment Name**: `supply-graph-ai` (or the active project-specific Python 3.12 env)
...
- Dependencies are managed via `requirements.txt` and the package config in `pyproject.toml`

## Project Structure (from maps)

The repository follows this structure:
- `src/cli/` - CLI entrypoints and command groups
- `src/config/` - Runtime and domain configuration
- `src/core/api/` - API routes and request/response models
- `src/core/generation/` - Repository mapping and BOM generation pipeline
- `src/core/matching/` - Matching rules and import/export services
- `src/core/services/` - Domain and system service layer
- `src/core/storage/` - Storage and migration services
- `src/core/models/` - Core data models
- `src/core/utils/` - Shared utility modules (logging, schema, secrets)
- `src/core/validation/` - Validation engine and rule sets
- `deploy/` - Cloud deployment providers and scripts
- `scripts/` - Repository/tooling automation scripts
- `synth/` - Synthetic data generation scripts
