...
!!! note "Both paths keep the API on this machine"

    `install.sh` and `docker-compose.yml` both bind the API port to
    `127.0.0.1`, so `http://localhost:8001` works on the machine you set the
    node up on and the API is not reachable from anywhere else. Nothing is
    lost by this: the web interface proxies `/v1` itself, over the container
    network, which is why the browser never needs that port.

    To use the node as an API server from another machine, publish it
    deliberately:

    ```bash
    OHM_API_BIND=0.0.0.0 sh install.sh        # installer
    echo "API_BIND=0.0.0.0" >> .env           # compose
    ```

    Before you do, set `API_KEYS` and read the node's write surface — parts of
    it still accept anonymous calls, which is tracked and being closed.
