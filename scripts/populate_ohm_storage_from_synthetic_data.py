...
Uses the same storage service and StorageOrganizer as the rest of OHM so the layout
matches what the matching service expects (okh/, okw/ at top level, recursive below).

Equivalent CLI command:
    ohm storage populate --provider azure_blob --bucket ohm --data-dir synthetic_data

Run from project root with the project venv active. Requires .env: STORAGE_PROVIDER=azure_blob,
AZURE_STORAGE_ACCOUNT, AZURE_STORAGE_KEY.

(Legacy deployments may still use container name ``ome``; pass ``--container ome`` if needed.)
"""
...
async def main() -> int:
    parser = argparse.ArgumentParser(
        description="Populate a blob container from synthetic_data/ using the storage service"
    )
    parser.add_argument(
        "--container",
        default="ohm",
        help="Target container / bucket name (default: ohm)",
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
