...
class DocValidator:
    ...
    def check_auth_header(self):
        ...
        code_checks_bearer = (
            'api_key.startswith("Bearer ")' in code_content
            or 'auth_header.startswith("Bearer ")' in code_content
        )
        ...
        doc_uses_x_api_key = (
            "X-API-Key" in doc_content and "Authorization" not in doc_content
        )
        doc_uses_bearer = (
            "Authorization: Bearer" in doc_content or "Bearer" in doc_content
        )
                ...
                if port_match:
                    ...
                    for doc_file in docs_dir.rglob("*.md"):
                        ...
                        with open(doc_file, "r") as f:
                            ...
                            api_port_pattern = re.compile(
                                r"(?:http://localhost:|:)(\d+)(?:/v1/api|/v1/api/)"
                            )
                            ...
                            for port in port_refs:
                                if (
                                    port != default_port
                                    and port not in other_service_ports
                                ):
                                    self.issues.append(
                                        {
                                            "type": "port_mismatch",
