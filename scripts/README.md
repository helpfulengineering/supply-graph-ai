...
| `diagnose_llm` | Report why a node will or will not use an LLM: stored credentials, whether each decrypts, and the availability answer generation actually uses. Run it inside a deployed container (API and worker separately) when Settings and generation disagree. | `python scripts/diagnose_llm.py` |
| `install_check` ✎ | Run the real install.sh against images built from this tree (via a throwaway local registry) and check the node an operator would get: containers healthy, identity minting works, keys on the mount and durable across recreate. | `make install-check   # or: OHM_CHECK_API_IMAGE=<tag> OHM_CHECK_WEB_IMAGE=<tag> sh scripts/install_check.sh` |
...
| `remove_supply_tree_storage` ✎ | Delete storage left by saved supply-tree solutions (#498); reports counts, --apply to commit. | `python scripts/remove_supply_tree_storage.py [--apply]` |
...
| `install` ✎ | Install and start a node on a host with Docker: mint secrets, resolve the latest release, start, wait for health, print the admin key. | `curl -fsSLO https://www.openhardwaremanager.org/install.sh && sha256sum -c install.sh.sha256 && sh install.sh` |
