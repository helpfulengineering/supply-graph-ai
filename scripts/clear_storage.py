...
async def main():
    ...
    if args.dry_run:
        ...
        logger.info("")
        logger.info(
            f"Total: {len(okh_files)} OKH + {len(okw_files)} OKW = {len(okh_files) + len(okw_files)} files"
        )
        ...
        logger.info("")
        logger.info(
            f"✅ Cleanup complete: {okh_count} OKH + {okw_count} OKW = {okh_count + okw_count} files deleted"
        )
