...
class EdgeCaseDetector:
    ...
    def detect_edge_cases(self, test_results: List[Dict]) -> Dict:
        ...
        edge_cases["case_statistics"] = self._calculate_case_statistics(
            edge_cases["detected_cases"]
        )
        ...
        edge_cases["recommendations"] = self._generate_edge_case_recommendations(
            edge_cases
        )
        ...
        for case_type, cases in detected_cases.items():
            if cases:
                statistics[case_type] = {
                    ...
                    "scenario_types_affected": list(
                        set(case["scenario_type"] for case in cases)
                    ),
                    "severity": self.edge_case_patterns.get(case_type, {}).get(
                        "severity", "unknown"
                    ),
                    "impact": self.edge_case_patterns.get(case_type, {}).get(
    ...
    def _detect_additional_patterns(self, test_results: List[Dict]) -> Dict:
        ...
        if (
            len(extreme_high) > len(confidence_scores) * 0.8
        ):  # More than 80% are extreme high
        ...
        if durations:
            ...
            slow_tests = [
                r for r in test_results if r.get("duration", 0) > avg_duration * 3
            ]
                    ...
                    "examples": [
                        {
                            ...
                            "error_message": (
                                ...
                                if len(r.get("error_message", "")) > 100
                    ...
                    critical_issues.append(
                        {
                            ...
                            "description": self.edge_case_patterns[case_type][
                                "description"
                            ],
                            "impact": self.edge_case_patterns[case_type]["impact"],
                            "affected_scenarios": [
                                case["scenario_id"] for case in cases[:5]
                            ],
                        }
                    )
        ...
        report = {
            ...
            "summary": {
                "total_edge_cases": sum(
                    len(cases) for cases in edge_cases["detected_cases"].values()
                ),
            ...
            "top_recommendations": edge_cases["recommendations"][
                :10
