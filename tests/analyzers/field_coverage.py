...
class FieldCoverageAnalyzer:
    ...
    def analyze_test_results(self, test_results: List[Dict]) -> Dict:
        ...
        coverage_data["unused_fields"]["okh"] = self._find_unused_fields(
            coverage_data["okh_fields_available"],
            coverage_data["okh_field_usage"],
            total_tests,
        )
        coverage_data["unused_fields"]["okw"] = self._find_unused_fields(
            coverage_data["okw_fields_available"],
            coverage_data["okw_field_usage"],
            total_tests,
        )
        ...
        coverage_data["field_effectiveness"] = self._calculate_field_effectiveness(
            test_results
        )
    ...
    def _calculate_utilization_rates(
        self, available_fields: Dict, used_fields: Dict
    ) -> Dict:
    ...
    def _find_unused_fields(
        self, available_fields: Dict, used_fields: Dict, total_tests: int
    ) -> List[str]:
    ...
    def _find_underutilized_fields(
        self, available_fields: Dict, used_fields: Dict, total_tests: int
    ) -> List[str]:
    ...
    def _analyze_critical_fields(
        ...
        for field in self.critical_okh_fields:
            ...
            critical_coverage["okh"][field] = {
                ...
                "utilization_rate": (
                    used_count / available_count if available_count > 0 else 0.0
                ),
                "is_well_covered": (
                    used_count / available_count >= 0.8
                    if available_count > 0
                    else False
                ),
            }
    ...
    def _calculate_field_effectiveness(self, test_results: List[Dict]) -> Dict:
        ...
        field_effectiveness = defaultdict(
            lambda: {"successful_matches": 0, "total_matches": 0}
        )

        for result in test_results:
            if (
                "field_usage" in result
                and "fields_used_in_matching" in result["field_usage"]
            ):
        ...
        for field, stats in field_effectiveness.items():
            if stats["total_matches"] > 0:
                effectiveness_rates[field] = {
                    "success_rate": stats["successful_matches"]
                    / stats["total_matches"],
    ...
    def _generate_recommendations(self, coverage_data: Dict) -> List[str]:
        ...
        unused_okh_critical = (
            set(coverage_data["unused_fields"]["okh"]) & self.critical_okh_fields
        )
        unused_okw_critical = (
            set(coverage_data["unused_fields"]["okw"]) & self.critical_okw_fields
        )
        ...
        underutilized_okh_critical = (
            set(coverage_data["underutilized_fields"]["okh"]) & self.critical_okh_fields
        )
        underutilized_okw_critical = (
            set(coverage_data["underutilized_fields"]["okw"]) & self.critical_okw_fields
        )
        ...
        low_effectiveness_fields = [
            field
            for field, stats in coverage_data["field_effectiveness"].items()
        ...
        all_unused = (
            coverage_data["unused_fields"]["okh"]
            + coverage_data["unused_fields"]["okw"]
        )
        ...
        poorly_covered_okh = [
            field
            for field, stats in okh_critical_coverage.items()
            if not stats["is_well_covered"] and stats["available_in_tests"] > 0
        ]

        poorly_covered_okw = [
            field
            for field, stats in okw_critical_coverage.items()
            if not stats["is_well_covered"] and stats["available_in_tests"] > 0
        ]
    ...
    def generate_coverage_report(self, test_results: List[Dict]) -> Dict:
        ...
        report = {
            ...
            "summary": {
                ...
                "underutilized_okh_fields": len(
                    coverage_data["underutilized_fields"]["okh"]
                ),
                "underutilized_okw_fields": len(
                    coverage_data["underutilized_fields"]["okw"]
                ),
                "critical_fields_well_covered": sum(
                    1
                    for field_stats in coverage_data["critical_field_coverage"][
                        "okh"
                    ].values()
                ...
                "total_critical_fields": len(self.critical_okh_fields)
                + len(self.critical_okw_fields),
            },
            "field_analysis": coverage_data,
            "top_recommendations": coverage_data["recommendations"][
                :5
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze field coverage from test results"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        default="field_coverage_report.json",
    ...
    print(
        f"Critical fields well covered: {summary['critical_fields_well_covered']}/{summary['total_critical_fields']}"
    )
