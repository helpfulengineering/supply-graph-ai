...
class MatchQualityAnalyzer:
    ...
    def analyze_match_quality(self, test_results: List[Dict]) -> Dict:
        ...
        quality_analysis["overall_metrics"] = self._calculate_overall_metrics(
            test_results
        )

        # Analyze confidence scores
        quality_analysis["confidence_analysis"] = self._analyze_confidence_scores(
            test_results
        )
        ...
        quality_analysis["scenario_type_analysis"] = self._analyze_by_scenario_type(
            test_results
        )

        # Identify false positives and negatives
        quality_analysis["false_positives"], quality_analysis["false_negatives"] = (
            self._identify_false_results(test_results)
        )

        # Analyze confidence calibration
        quality_analysis["confidence_calibration"] = (
            self._analyze_confidence_calibration(test_results)
        )

        # Calculate performance metrics
        quality_analysis["performance_metrics"] = self._calculate_performance_metrics(
            test_results
        )

        # Generate quality recommendations
        quality_analysis["quality_recommendations"] = (
    ...
    def _calculate_overall_metrics(self, test_results: List[Dict]) -> Dict:
        ...
        outcome_counts = Counter(
            r.get("actual_outcome", "unknown") for r in test_results
        )
    ...
    def _analyze_confidence_scores(self, test_results: List[Dict]) -> Dict:
        ...
        confidence_analysis["score_distribution"] = {
            "excellent": len(
                [
                    s
                    for s in scores
                    if s >= self.quality_thresholds["excellent_confidence"]
                ]
            ),
            "good": len(
                [
                    s
                    for s in scores
                    if self.quality_thresholds["good_confidence"]
                    <= s
                    < self.quality_thresholds["excellent_confidence"]
                ]
            ),
            "acceptable": len(
                [
                    s
                    for s in scores
                    if self.quality_thresholds["acceptable_confidence"]
                    <= s
                    < self.quality_thresholds["good_confidence"]
                ]
            ),
            "poor": len(
                [
                    s
                    for s in scores
                    if s < self.quality_thresholds["acceptable_confidence"]
                ]
            ),
        }
        ...
        for category, threshold in self.quality_thresholds.items():
            if category == "excellent_confidence":
                category_scores = [
                    d for d in confidence_data if d["confidence"] >= threshold
                ]
                ...
                category_scores = [
                    ...
                    for d in confidence_data
                    if d["confidence"]
                    < self.quality_thresholds["acceptable_confidence"]
                ]

            if category_scores:
                success_rate = sum(1 for d in category_scores if d["success"]) / len(
                    category_scores
                )
                confidence_analysis["quality_categories"][category] = {
                    ...
                    "avg_confidence": statistics.mean(
                        [d["confidence"] for d in category_scores]
                    ),
                }
        ...
        confidence_analysis["anomalies"] = self._detect_confidence_anomalies(
            confidence_data
        )
    ...
    def _analyze_accuracy(self, test_results: List[Dict]) -> Dict:
        ...
        if true_positives + false_positives > 0:
            accuracy_analysis["precision"] = true_positives / (
                true_positives + false_positives
            )

        if true_positives + false_negatives > 0:
            accuracy_analysis["recall"] = true_positives / (
                true_positives + false_negatives
            )

        if accuracy_analysis["precision"] + accuracy_analysis["recall"] > 0:
            accuracy_analysis["f1_score"] = (
                2
                * (accuracy_analysis["precision"] * accuracy_analysis["recall"])
                / (accuracy_analysis["precision"] + accuracy_analysis["recall"])
            )
    ...
    def _analyze_confidence_calibration(self, test_results: List[Dict]) -> Dict:
        ...
        for result in test_results:
            ...
            bin_key = (
                f"{int(confidence * 10) / 10:.1f}-{int(confidence * 10) / 10 + 0.1:.1f}"
            )
    ...
    def _detect_confidence_anomalies(self, confidence_data: List[Dict]) -> List[Dict]:
        ...
        if len(scores) > 1:
            ...
            for d in confidence_data:
                z_score = (
                    abs(d["confidence"] - mean_score) / std_dev if std_dev > 0 else 0
                )
    ...
    def generate_quality_report(self, test_results: List[Dict]) -> Dict:
        ...
        report = {
            ...
            "summary": {
                "overall_success_rate": quality_analysis["overall_metrics"].get(
                    "success_rate", 0
                ),
                "overall_accuracy": quality_analysis["accuracy_analysis"].get(
                    "accuracy_rate", 0
                ),
                "average_confidence": quality_analysis["overall_metrics"]
                .get("confidence_statistics", {})
                .get("mean", 0),
                "calibration_error": quality_analysis["confidence_calibration"].get(
                    "calibration_error", 0
                ),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze match quality from test results"
    )
    parser.add_argument(
        "--test-results", required=True, help="JSON file containing test results"
    )
    parser.add_argument(
        "--output",
        default="match_quality_report.json",
