"""
Simple real integration tests for validation framework.

This module tests the validation framework with actual domain services
without requiring the domain registry to be populated.
"""

import pytest
import asyncio
from src.core.validation import (
    ValidationEngine, ValidationResult, ValidationContext,
...
class TestSimpleRealValidationIntegration:
    """Test validation framework with real domain services (no domain registry dependency)"""
    ...
    def test_cooking_validation_rules_with_real_data(self):
        ...
        # Test with real recipe data (including allergen_info for commercial level)
        real_recipe_data = {
            ...
            "allergen_info": ["gluten", "dairy"]  # Added for commercial level
        }
        ...
        expected_missing_professional = ['nutritional_info', 'food_safety_notes']
        assert sorted(professional_missing) == sorted(expected_missing_professional), \
            f"Professional level missing fields mismatch. Expected: {expected_missing_professional}, Got: {professional_missing}"
    
    @pytest.mark.asyncio
    async def test_manufacturing_okh_validator_with_real_data(self):
        """Test ManufacturingOKHValidator with real OKH data (no context)"""
        ...
        # Test professional level validation (with context)
        with patch('src.core.registry.domain_registry.DomainRegistry.list_domains', return_value=["manufacturing", "cooking"]):
        ...
        incomplete_okh_data = {
            "title": "Simple Sensor",
            "version": "1.0.0",  # Fixed version format
    ...
    async def test_cooking_recipe_validator_with_real_data(self):
        """Test CookingRecipeValidator with real recipe data (no context)"""
        ...
        complete_recipe_data = {
            ...
            "food_safety_notes": "Ensure proper food handling and storage procedures"
        }
        ...
        result = await validator.validate(basic_recipe_data, None)
        assert isinstance(result, ValidationResult)
        assert result.valid is True, f"Home cooking validation failed: {result.errors}"
    
    @patch('src.core.validation.factory.DomainRegistry.list_domains', return_value=["manufacturing", "cooking"])
    @patch('src.core.validation.factory.DomainRegistry.get_domain_metadata', return_value=MagicMock())
    def test_validation_context_factory_with_real_domains(self, mock_get_metadata, mock_list_domains):
        """Test ValidationContextFactory with real domain detection"""
        # Test domain keyword detection using the correct method name
        manufacturing_data = {
            "okh": "test",
            "manufacturing": "data",
            "cnc": "machine",
        ...
        detected_domain = ValidationContextFactory.create_context_from_detection(
            manufacturing_data, {}, "professional"
        )
        assert detected_domain.domain == "manufacturing"
        assert detected_domain.quality_level == "professional"
    ...
    async def test_validation_engine_with_real_validators(self):
        """Test ValidationEngine with real domain validators (no context)"""
        ...
        result = await engine.validate(recipe_data, "recipe")
        assert isinstance(result, ValidationResult)
    ...
    def test_validation_rules_edge_cases(self):
        """Test validation rules with edge cases and boundary conditions"""
        rules = ManufacturingValidationRules()
        
        # Test with empty data
        empty_data = {}
        hobby_missing = rules.get_missing_required_fields(empty_data, "hobby")
        assert len(hobby_missing) == 6, f"Empty data should have 6 missing hobby fields, got {len(hobby_missing)}"
        ...
        hobby_missing = rules.get_missing_required_fields(none_data, "hobby")
        assert "title" in hobby_missing, "None values should be considered missing"
        ...
        hobby_missing = rules.get_missing_required_fields(partial_data, "hobby")
        assert len(hobby_missing) == 4, f"Partial data should have 4 missing hobby fields, got {len(hobby_missing)}"
        
        # Test quality level validation
        assert rules.validate_quality_level("hobby") is True
        assert rules.validate_quality_level("professional") is True
        assert rules.validate_quality_level("medical") is True
        assert rules.validate_quality_level("invalid") is False
