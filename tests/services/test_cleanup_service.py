...
from src.core.services.cleanup_service import CleanupOptions, CleanupService
...
async def _create_scaffold_with_template_level(
    tmp_path: Path, template_level: str
) -> Path:
...
async def test_cleanup_with_links_and_bridge_pages(tmp_path):
    ...
    assert (
        "../../bom/index.md" in bridge_content
        or "bom/index.md" in bridge_content.lower()
    )
    ...
    assert not docs_index.exists() or any(
        str(docs_index) == p for p in res2.removed_files
    )
    assert not bom_index.exists() or any(
        str(bom_index) == p for p in res2.removed_files
    )
    assert not bridge_bom.exists() or any(
        str(bridge_bom) == p for p in res2.removed_files
    )
...
async def test_cleanup_detects_broken_links(tmp_path):
    ...
    bom_was_removed = (
        any(str(bom_index) == p for p in res.removed_files) or not bom_index.exists()
    )
    ...
    if bom_was_removed:
        ...
        assert (
            len(broken_link_warnings) > 0
        ), "Expected broken link warning when linked file is removed"
        ...
        assert (
            "bom" in warnings_text.lower()
        ), "Warning should mention broken link target"
    else:
