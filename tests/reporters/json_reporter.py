...
class JSONReporter:
    ...
    def generate_comprehensive_report(
        self, test_results: List[Dict], test_run_metadata: Optional[Dict] = None
    ) -> Dict:
        """Generate a report combining all analyses"""

        # Run all analyses
        print("Running field coverage analysis...")
        field_coverage_report = self.field_analyzer.generate_coverage_report(
            test_results
        )

        print("Running edge case detection...")
        edge_case_report = self.edge_case_detector.generate_edge_case_report(
            test_results
        )
    ...
    def _generate_summary_statistics(self, test_results: List[Dict]) -> Dict:
        ...
        for result in test_results:
            ...
            if scenario_type not in scenario_type_stats:
                scenario_type_stats[scenario_type] = {
                    "total": 0,
                    "successful": 0,
                    "failed": 0,
                }
    ...
    def _generate_executive_summary(
        self,
        summary_stats: Dict,
        field_coverage_report: Dict,
        edge_case_report: Dict,
        quality_report: Dict,
    ) -> Dict:
        ...
        key_metrics = {
            ...
            "match_accuracy": quality_report.get("summary", {}).get(
                "overall_accuracy", 0
            ),
            "field_coverage_rate": self._calculate_field_coverage_rate(
                field_coverage_report
            ),
            "edge_cases_detected": edge_case_report.get("summary", {}).get(
                "total_edge_cases", 0
            ),
            "critical_issues": edge_case_report.get("summary", {}).get(
                "critical_issues", 0
            ),
        }
    ...
    def _calculate_field_coverage_rate(self, field_coverage_report: Dict) -> float:
        ...
        total_fields = summary.get("total_okh_fields", 0) + summary.get(
            "total_okw_fields", 0
        )
        unused_fields = summary.get("unused_okh_fields", 0) + summary.get(
            "unused_okw_fields", 0
        )
    ...
    def _identify_top_priorities(
        self, field_coverage_report: Dict, edge_case_report: Dict, quality_report: Dict
    ) -> List[Dict]:
        """Identify top priorities for improvement"""
        priorities = []

        # Critical issues from edge case analysis
        critical_issues = edge_case_report.get("edge_case_analysis", {}).get(
            "critical_issues", []
        )
        ...
        field_recommendations = field_coverage_report.get("field_analysis", {}).get(
            "recommendations", []
        )
        ...
        # Quality issues
        quality_recommendations = quality_report.get("quality_analysis", {}).get(
    ...
    def _generate_integrated_recommendations(
        ...
        field_recs = field_coverage_report.get("field_analysis", {}).get(
            "recommendations", []
        )
        ...
        edge_recs = edge_case_report.get("edge_case_analysis", {}).get(
            "recommendations", []
        )
        ...
        quality_recs = quality_report.get("quality_analysis", {}).get(
            "quality_recommendations", []
        )
        ...
        for rec in all_recommendations:
            ...
            grouped_recommendations[key]["recommendations"].append(
                rec["recommendation"]
            )
    ...
    def save_report(self, report: Dict, output_file: str):
        ...
        print(
            f"Success Rate: {executive_summary.get('key_metrics', {}).get('overall_success_rate', 0):.2%}"
        )
        print(
            f"Match Accuracy: {executive_summary.get('key_metrics', {}).get('match_accuracy', 0):.2%}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate comprehensive JSON report from test results"
    )
    parser.add_argument(
        "--test-results", required=True, help="JSON file containing test results"
    )
    parser.add_argument(
        "--output", default="comprehensive_report.json", help="Output file for report"
    )
