...
class TestSupplyTreePerformance:
    ...
    def test_serialization_performance(self):
        ...
        print(
            f"  - Average per object: {(serialization_time + deserialization_time) / 1000 * 1000:.2f}ms"
        )

        # Should be very fast (less than 2 seconds for 1000 objects)
        assert (
            serialization_time + deserialization_time
        ) < 2.0, "Serialization too slow"
    ...
    def test_set_operations_performance(self):
        ...
        print(
            f"  - Set operations (intersection, union, difference): {set_operations_time:.4f}s"
        )
    ...
    def test_memory_usage(self):
        ...
        assert (
            set_memory_used < 50
        ), f"Set memory usage too high: {set_memory_used:.2f} MB"
    ...
    def test_api_response_size(self):
        ...
        print(
            f"  - Response size: {response_size:,} bytes ({response_size_kb:.2f} KB, {response_size_mb:.2f} MB)"
        )
...
class TestPerformanceComparison:
    ...
    def test_theoretical_complexity_reduction(self):
        ...
        time_improvement = (
            (estimated_complex_time - simplified_time) / estimated_complex_time * 100
        )
        memory_improvement = (
            (estimated_complex_memory - len(str(simplified_data)))
            / estimated_complex_memory
            * 100
        )
        ...
        assert (
            time_improvement > 50
        ), f"Time improvement too low: {time_improvement:.1f}%"
        assert (
            memory_improvement > 50
