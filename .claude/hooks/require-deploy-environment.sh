#!/usr/bin/env bash
# PreToolUse(Bash) guard: refuse to run a deploy_azure*.py script (backend
# deploy_azure.py, deploy_azure_frontend.py, …) without an explicit
...
if printf '%s' "$cmd" | grep -qE '(python[0-9]*|uv run)[^&|;]*deploy_azure[a-z_]*\.py'; then
  ...
  if ! printf '%s' "$cmd" | grep -qE '(^|[[:space:]])--environment([[:space:]]|=)'; then
    echo "Blocked: a deploy_azure*.py script was invoked without --environment. The deploy target must be explicit — importing src.config runs load_dotenv(), so a local .env (ENVIRONMENT=development) can silently redirect the deploy and mis-point prod. Re-run with an explicit target, e.g. --environment production." >&2
    exit 2
  fi
fi
