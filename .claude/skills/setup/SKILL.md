...
# Edit this file to change configuration. See .env.example for all options.
...
Note on `.env.example`: the block between its `BEGIN/END GENERATED` markers is
...
### Environment files
- `.env.example` — canonical template with all params documented; copy to `.env`
