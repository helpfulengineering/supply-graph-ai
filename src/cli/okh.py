...
@click.argument("manifest_id", type=str)
@standard_cli_command(
    help_text="Show the authorship/publication provenance recorded for a manifest.",
    epilog="""
    Examples:
      ohm okh provenance 123e4567-e89b-12d3-a456-426614174000
    """,
    async_cmd=True,
    track_performance=True,
    handle_errors=True,
...
async def provenance(
    ...
    """Show provenance for an OKH manifest."""
    cli_ctx = ctx.obj
    cli_ctx.start_command_tracking("okh-provenance")
    try:
        response = await cli_ctx.api_client.request(
            "GET", f"/api/okh/{manifest_id}/provenance"
        )
...
@standard_cli_command(
    ...
    filesystem output, and zip with an explicit --output-path, write to the
    SERVER's filesystem — only meaningful when the server you're talking to
    (OHM_SERVER_URL) is your own machine. The server refuses both with a 422
    unless its operator has set SCAFFOLD_OUTPUT_ROOT, and even then only a
    path inside that configured root is accepted (#512).
    ...
    Examples:
      ...
      # Generate to filesystem with custom organization (server-local; requires
      # SCAFFOLD_OUTPUT_ROOT on the server, and --output-path inside it)
...
@standard_cli_command(
    ...
    project_path is resolved on the SERVER's filesystem — only meaningful
    when the server you're talking to (OHM_SERVER_URL) is your own machine.
    The server refuses this with a 422 unless its operator has set
    SCAFFOLD_OUTPUT_ROOT, and even then only a path inside that configured
    root is accepted (#512).
    """,
    epilog="""
    Examples:
