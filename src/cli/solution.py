...
@click.option("--id", "solution_id", help="Solution ID (UUID). If not provided, a new UUID will be generated.")
@click.option("--ttl-days", "ttl_days", type=int, help="Time-to-live in days for the solution")
...
async def save(
    ...
    try:
        ...
        response = await cli_ctx.api_client.request("POST", endpoint, json_data=request_body)
...
@click.option("--output", "-o", type=click.Path(), help="Output file path. If not provided, outputs to stdout.")
@click.pass_context
@standard_cli_command(
    help_text="""
...
@click.option("--matching-mode", type=click.Choice(["single-level", "nested"]), help="Filter by matching mode")
@click.option("--sort-by", type=click.Choice(["created_at", "updated_at", "score", "age_days"]), help="Sort field")
        ...
        if output_format == "json":
            ...
            if not solutions:
                ...
                for sol in solutions:
                    ...
                    title = sol.get("okh_title") or sol.get("metadata", {}).get("okh_title", "N/A")
...
@solution_group.command()
@click.argument("solution_id", type=str)
@click.option("--days", type=int, default=30, help="Additional days to extend TTL (default: 30)")
@click.pass_context
@standard_cli_command(
    help_text="""
...
@solution_group.command()
@click.option("--max-age-days", type=int, help="Maximum age in days for solutions to keep")
@click.option("--before-date", help="Delete solutions created before this date (YYYY-MM-DD)")
@click.option("--dry-run", is_flag=True, help="Show what would be deleted without actually deleting")
@click.option("--archive", is_flag=True, help="Archive stale solutions instead of deleting them")
@click.pass_context
@standard_cli_command(
    help_text="""
...
async def cleanup(
    ...
    try:
        ...
        if output_format == "json":
            ...
            if is_dry_run:
                click.echo(f"🔍 Dry run: Would {'archive' if archive else 'delete'} {deleted_count} solution(s)")
            else:
