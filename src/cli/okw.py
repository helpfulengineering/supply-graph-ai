...
@okw_group.command(name="inventory")
@standard_cli_command(
    help_text="Enumerate every facility without reading any of them.",
    async_cmd=True,
)
@click.pass_context
async def inventory_cmd(
    ...
    service = await OKWService.get_instance()
        ...
        if facilities:
            ...
            if output_format == "json":
                ...
                for facility in facilities:
                    ...
                    click.echo(
                        f"   Location: {facility.get('location', {}).get('address', 'Unknown')}"
                    )
    ...
    try:
        if not force:
            if not click.confirm(
                f"Are you sure you want to delete facility {facility_id}?"
            ):
...
async def extract_capabilities(
    ...
    try:
        ...
        # Create request data with LLM configuration
        request_data = create_llm_request_data(cli_ctx, {"content": facility_data})
        ...
        async def http_extract():
            ...
            response = await cli_ctx.api_client.request(
                "POST", "/api/okw/extract", json_data=request_data
            )
        ...
        if capabilities:
            ...
            if output_format == "json":
                ...
                for i, cap in enumerate(capabilities, 1):
                    ...
                    cap_limitations = (
                        cap.get("limitations", {}) if isinstance(cap, dict) else {}
                    )
...
async def upload(
    ...
    try:
        ...
        if isinstance(result, dict):
            facility_id = (
                result.get("id")
                or (result.get("okw", {}) or {}).get("id")
                or (result.get("facility", {}) or {}).get("id")
            )
        else:
            facility_id = getattr(result, "id", None) or getattr(
                getattr(result, "okw", None), "id", None
            )
        ...
        if facility_id:
            ...
            cli_ctx.log(
                f"⚠️  Upload completed but no facility ID found in response", "warning"
            )
            if cli_ctx.verbose:
                cli_ctx.log(f"Response structure: {type(result).__name__}", "info")
                cli_ctx.log(
                    f"Response keys/attrs: {list(result.keys()) if isinstance(result, dict) else dir(result)[:10]}",
                    "info",
                )
...
async def search(
    ...
    try:
        ...
        async def http_search():
            ...
            response = await cli_ctx.api_client.request(
                "GET", "/api/okw/search", params=params
            )
            return response
        ...
        async def fallback_search():
            ...
            return {
                "facilities": [
                    facility.to_dict() for facility in filtered_facilities[:limit]
                ],
        ...
        if facilities:
            ...
            if output_format == "json":
                ...
                for i, facility in enumerate(facilities, 1):
                    cli_ctx.log(
                        f"{i}. {facility.get('name', 'Unknown')} - {facility.get('location', 'Unknown location')}",
                        "info",
                    )
        else:
...
async def fix(
    ...
    try:
        ...
        validation_result = validate_okw_facility(
            facility_data, quality_level=quality_level, strict_mode=strict_mode
        )
        ...
        cli_ctx.log(
            f"Warnings fixed: {fix_report.warnings_fixed}",
            "success" if fix_report.warnings_fixed > 0 else "info",
        )
        cli_ctx.log(
            f"Errors fixed: {fix_report.errors_fixed}",
            "success" if fix_report.errors_fixed > 0 else "info",
        )
                ...
                if low_confidence_fixes:
                    ...
                    if click.confirm("Apply low-confidence fixes?"):
                        ...
                        for fix in low_confidence_fixes:
                            try:
                                from ..core.validation.auto_fix import (
                                    _apply_fix_to_facility,
                                )
                                ...
                                applied_count += 1
                                cli_ctx.log(
                                    f"Applied fix: {fix.description}", "success"
                                )
                            except Exception as e:
                                cli_ctx.log(
                        ...
                        if applied_count > 0:
                            cli_ctx.log(
                                f"✅ Applied {applied_count} additional fixes",
                                "success",
                            )
                        else:
        ...
        if dry_run:
            ...
            if status == "complete_success":
                ...
                cli_ctx.log(
                    f"\n⚠️  Partial success: Some issues fixed, but {fix_report.remaining_warnings} warnings and {fix_report.remaining_errors} errors remain",
                    "warning",
                )
                ...
                if fix_report.remaining_errors > 0:
                    cli_ctx.log(
                        f"❌ {fix_report.remaining_errors} error(s) remain", "error"
                    )
                if fix_report.remaining_warnings > 0:
                    cli_ctx.log(
                        f"⚠️  {fix_report.remaining_warnings} warning(s) remain",
                        "warning",
                    )
                cli_ctx.log(
                    f"⚠️  Facility saved to: {output_file} (unchanged or partially fixed)",
                    "warning",
                )
                    ...
                    for i, error in enumerate(final_validation.errors, 1):
                        ...
                        if isinstance(error, str):
                            ...
                            if hasattr(error, "suggestion") and error.suggestion:
                                cli_ctx.log(
                                    f"     Suggestion: {error.suggestion}", "info"
                                )
                ...
                if final_validation.warnings:
                    ...
                    for i, warning in enumerate(final_validation.warnings, 1):
                        ...
                        if isinstance(warning, str):
                            ...
                            cli_ctx.log(
                                f"  {i}. [{warning_code}] {warning_msg}", "warning"
                            )
                            ...
                            if hasattr(warning, "suggestion") and warning.suggestion:
                                cli_ctx.log(
                                    f"     Suggestion: {warning.suggestion}", "info"
                                )
    ...
    try:
        ...
        if not storage_service.manager:
            ...
            raise click.ClickException(
                "Storage service is not configured. Please configure storage first."
            )
        ...
        async for obj in storage_service.manager.list_objects(prefix=prefix):
            ...
            if key.endswith((".json", ".yaml", ".yml")):
                ...
                try:
                    ...
                    if cli_ctx.verbose:
                        cli_ctx.log(
                            f"Warning: Could not extract facility_id from {key}: {str(e)}",
                            "warning",
                        )
        ...
        # Prepare output
        result = {"files": files, "total": len(files), "prefix": prefix}
        ...
        if output_format == "json" or format == "json":
            ...
            if files:
                ...
                for i, file_info in enumerate(files, 1):
                    ...
                    click.echo(
                        f"     Size: {size_kb:.1f} KB | Modified: {modified_str}"
                    )
