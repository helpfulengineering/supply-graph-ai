...
async def _display_metrics_results(cli_ctx: CLIContext, result: dict, output_format: str, endpoint: Optional[str] = None):
    ...
    if output_format == "json":
        ...
        if endpoint:
            ...
            click.echo(f"  Avg Processing Time: {metrics_data.get('avg_processing_time_ms', 0):.2f} ms")
            
            if "p95_processing_time_ms" in metrics_data:
                click.echo(f"  P95 Processing Time: {metrics_data.get('p95_processing_time_ms', 0):.2f} ms")
            if "p99_processing_time_ms" in metrics_data:
                click.echo(f"  P99 Processing Time: {metrics_data.get('p99_processing_time_ms', 0):.2f} ms")
            if "min_processing_time_ms" in metrics_data:
                click.echo(f"  Min Processing Time: {metrics_data.get('min_processing_time_ms', 0):.2f} ms")
            if "max_processing_time_ms" in metrics_data:
                click.echo(f"  Max Processing Time: {metrics_data.get('max_processing_time_ms', 0):.2f} ms")
            ...
            if "total_llm_cost" in metrics_data and metrics_data.get("total_llm_cost", 0) > 0:
                click.echo(f"  Total LLM Cost: ${metrics_data.get('total_llm_cost', 0):.4f}")
            if "total_llm_tokens" in metrics_data and metrics_data.get("total_llm_tokens", 0) > 0:
                click.echo(f"  Total LLM Tokens: {metrics_data.get('total_llm_tokens', 0):,}")
        else:
            ...
            if "endpoints" in metrics_data:
                ...
                click.echo(f"  Recent Requests (1h): {summary.get('recent_requests_1h', 0)}")
                click.echo(f"  Active Requests: {summary.get('active_requests', 0)}")
                click.echo(f"  Endpoints Tracked: {summary.get('endpoints_tracked', 0)}")
                
                if endpoints:
                    click.echo("\nEndpoint Metrics:")
                    for endpoint_key, endpoint_metrics in endpoints.items():
                        click.echo(f"  {endpoint_key}")
                        click.echo(f"    Requests: {endpoint_metrics.get('total_requests', 0)}")
                        click.echo(f"    Success Rate: {endpoint_metrics.get('success_rate', 0):.2f}%")
                        click.echo(f"    Avg Time: {endpoint_metrics.get('avg_processing_time_ms', 0):.2f} ms")
                        if endpoint_metrics.get('total_llm_cost', 0) > 0:
                            click.echo(f"    LLM Cost: ${endpoint_metrics.get('total_llm_cost', 0):.4f}")
            else:
                    ...
                    if operation_stats:
                        ...
                        for operation, stats in operation_stats.items():
                            if isinstance(stats, dict):
                                ...
                                click.echo(f"    {operation}: {count} operations, avg {avg_duration:.2f} ms")
...
@click.option('--summary/--no-summary', default=True, help='Show summary only (default: True)')
@standard_cli_command(
    help_text="""
...
@click.pass_context
async def metrics(ctx, endpoint: Optional[str], summary: bool,
                 verbose: bool, output_format: str, use_llm: bool,
                 llm_provider: str, llm_model: Optional[str],
                 quality_level: str, strict_mode: bool):
    ...
    try:
        async def http_metrics():
            ...
            response = await cli_ctx.api_client.request("GET", "/api/utility/metrics", params=params)
            return response
        
        async def fallback_metrics():
            """Fallback - metrics are only available via API"""
            cli_ctx.log("Metrics are only available when connected to the API server.", "warning")
            cli_ctx.log("Please ensure the OME server is running and accessible.", "info")
