...
class ProjectAuditService:
    ...
    def _is_stub_content(self, content: str) -> bool:
        ...
        if any(pattern.lower() in content_lower for pattern in stub_patterns):
            ...
            placeholder_lines = sum(1 for line in lines if any(pattern.lower() in line.lower() for pattern in stub_patterns))
            ...
            if placeholder_lines / len(lines) >= 0.4 or (len(lines) <= 3 and placeholder_lines > 0):
                return True
