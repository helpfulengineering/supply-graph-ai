...
class BOMResolutionService:
    ...
    def _detect_bom_type(self, okh_manifest: OKHManifest) -> str:
        ...
        if okh_manifest.bom:
            if isinstance(okh_manifest.bom, str) and okh_manifest.bom.strip():
                return "external"
            elif isinstance(okh_manifest.bom, dict) and "external_file" in okh_manifest.bom:
                return "external"
    ...
    def _resolve_bom_path(self, bom_path: str, manifest_path: Optional[str] = None) -> str:
        """
    ...
    async def _load_external_bom(
        ...
        # Load file content
        file_content = await self._load_file_content(resolved_path, okh_service, manifest_path)
        ...
        if file_ext == ".json":
            ...
            raise ValueError(f"Unsupported BOM file format: {file_ext}. Only JSON and YAML are supported.")
        ...
        if isinstance(bom_data, dict):
            ...
            if "components" in bom_data or "name" in bom_data:
                ...
                raise ValueError(f"Invalid BOM file format: missing 'components' or 'name' field")
        else:
            raise ValueError(f"Invalid BOM file format: expected dict, got {type(bom_data)}")
    ...
    def _part_to_component(self, part: PartSpec) -> Component:
        ...
        if hasattr(part, "source") and part.source:
            metadata["source"] = part.source if isinstance(part.source, list) else [part.source]
        if hasattr(part, "export") and part.export:
            metadata["export"] = part.export if isinstance(part.export, list) else [part.export]
    ...
    def _sub_part_to_component(self, sub_part: Dict[str, Any]) -> Component:
        ...
        for key, value in sub_part.items():
            if key not in ["name", "quantity", "unit", "id", "sub_parts", "reference", "requirements"]:
                metadata[key] = value
    ...
    async def resolve_bom(
        ...
        if bom_type == "embedded":
            ...
            try:
                external_bom = await self._load_external_bom(okh_manifest, service, manifest_path)
                        ...
                        return BillOfMaterials(
                            ...
                            metadata={
                                **(external_bom.metadata or {}),
                                ...
                                "external_component_count": len(external_bom.components),
                                "embedded_component_count": len(embedded_bom.components)
                            }
                        )
                    else:
                        logger.info(
                            f"Successfully loaded external BOM with {len(external_bom.components)} components",
                            extra={"okh_id": str(okh_manifest.id), "bom_name": external_bom.name}
                        )
                        return external_bom
                else:
                    logger.warning(
                ...
                logger.warning(
                    ...
                    extra={"okh_id": str(okh_manifest.id), "error_type": type(e).__name__}
                )
    ...
    async def resolve_component_reference(
        ...
        if "path" in reference:
            ...
            try:
                ...
                if manifest:
                    logger.debug(f"Resolved component reference by path: {resolved_path}")
                    return manifest
                else:
        ...
        logger.warning(f"Unsupported component reference format: {list(reference.keys())}")
        return None
    ...
    async def _load_manifest_from_path(
        ...
        if os.path.exists(file_path):
            try:
                ...
                logger.warning(f"Failed to load OKH manifest from filesystem {file_path}: {e}")
        ...
        if okh_service and hasattr(okh_service, "load_file"):
            try:
                ...
                logger.warning(f"Failed to load OKH manifest via OKHService {file_path}: {e}")
                ...
                try:
                    ...
                    if okh_manifest:
                        ...
                        if (current_depth + 1) < max_depth:
                            try:
                                nested_bom = await self.resolve_bom(okh_manifest, service)
