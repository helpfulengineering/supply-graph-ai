...
class UUIDValidator:
    ...
    def fix_invalid_uuid(uuid_string: str, fallback_to_random: bool = False) -> str:
        ...
        if re.match(r'^[0-9a-f]{8}[0-9a-f]{4}[0-9a-f]{4}[0-9a-f]{4}[0-9a-f]{12}$', cleaned.replace('-', '')):
    ...
    def validate_and_fix_okw_data(okw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'id' in fixed_data:
            ...
            if not UUIDValidator.is_valid_uuid(original_id):
                ...
                fixed_id = UUIDValidator.fix_invalid_uuid(original_id, fallback_to_random=True)
