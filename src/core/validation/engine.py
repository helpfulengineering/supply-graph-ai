...
class ValidationEngine:
    ...
    async def validate(
        ...
        try:
            ...
            if context:
                domain_result = await self._validate_with_domain_context(
                    data, validation_type, context
                )
            ...
            result.add_error(
                f"Validation engine error: {str(e)}", code="validation_engine_error"
            )
            raise ValidationEngineError(f"Validation failed: {str(e)}") from e

        return result

    async def _validate_with_domain_context(
        self, data: Any, validation_type: str, context: ValidationContext
    ) -> ValidationResult:
        ...
        try:
            ...
            if hasattr(domain_validator, "validate"):
                ...
                if isinstance(validation_result, dict):
                    ...
                    if not validation_result:
                        result.add_error(
                            "Domain validation failed", code="domain_validation_failed"
                        )
                else:
            ...
            result.add_error(
                f"Domain validation error: {str(e)}", code="domain_validation_error"
            )
            raise DomainValidationError(f"Domain validation failed: {str(e)}") from e
        except Exception as e:
            result.add_error(
                f"Unexpected domain validation error: {str(e)}",
                code="domain_validation_unexpected",
            )
    ...
    async def _apply_context_validation(
    ...
    async def _apply_custom_rules(
        self, data: Any, custom_rules: Dict[str, Any]
    ) -> ValidationResult:
    ...
    def _convert_dict_to_validation_result(
        ...
        if "completeness_score" in validation_dict:
            result.metadata["completeness_score"] = validation_dict[
                "completeness_score"
            ]
    ...
    async def _run_validator(
        self, validator: Any, data: Any, context: Optional[ValidationContext]
    ) -> ValidationResult:
        ...
        try:
            if hasattr(validator, "validate"):
                ...
                if isinstance(validation_result, ValidationResult):
                    ...
                    if not validation_result:
                        result.add_error(
                            "Validator returned false", code="validator_failed"
                        )
                    return result
                else:
                    # Unknown return type
    ...
    def register_validator(
        self, validation_type: str, validator: Any, priority: int = 0
    ):
