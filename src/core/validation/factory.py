...
class ValidationContextFactory:
    ...
    def create_context(domain_name: str, 
        ...
        if domain_name not in DomainRegistry.list_domains():
            raise ValidationContextError(f"Domain '{domain_name}' not found. Available domains: {DomainRegistry.list_domains()}")
    ...
    @staticmethod
    def create_manufacturing_context(quality_level: str = "professional", 
                                   strict_mode: bool = False) -> ValidationContext:
    ...
    @staticmethod
    def create_cooking_context(quality_level: str = "home", 
                             strict_mode: bool = False) -> ValidationContext:
    ...
    @staticmethod
    def create_context_from_detection(requirements: Any, 
                                    capabilities: Any,
        ...
        detected_domain = ValidationContextFactory._detect_domain(requirements, capabilities)
        
        if detected_domain:
            return ValidationContextFactory.create_context(detected_domain, quality_level)
        else:
            # Default to manufacturing if detection fails
            return ValidationContextFactory.create_context("manufacturing", quality_level)
    ...
    def _detect_domain(requirements: Any, capabilities: Any) -> Optional[str]:
        ...
        if isinstance(requirements, dict):
            ...
            manufacturing_keywords = ["okh", "okw", "manufacturing", "hardware", "machining", "tolerance"]
            if any(keyword in str(requirements).lower() for keyword in manufacturing_keywords):
                return "manufacturing"
            
            # Check for cooking keywords
            cooking_keywords = ["recipe", "cooking", "kitchen", "ingredient", "food", "meal"]
            if any(keyword in str(requirements).lower() for keyword in cooking_keywords):
                return "cooking"
        
        if isinstance(capabilities, dict):
            # Check capabilities for domain indicators
            manufacturing_capabilities = ["cnc", "3d printing", "machining", "assembly"]
            if any(cap in str(capabilities).lower() for cap in manufacturing_capabilities):
                return "manufacturing"
    ...
    def validate_quality_level(domain_name: str, quality_level: str) -> bool:
        ...
        available_levels = ValidationContextFactory.get_available_quality_levels(domain_name)
