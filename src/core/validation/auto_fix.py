...
from .model_validator import validate_okh_manifest, validate_okw_facility, ValidationResult
...
def auto_fix_okh_manifest(
    ...
    fix_order = ["typo", "case_error", "data_movement", "data_transformation", "field_removal", "field_addition"]
            ...
            for fix in fixes_by_type[fix_type]:
                if fix.confidence >= fix_confidence_threshold:
                    try:
                        ...
                        logger.warning(f"Failed to apply fix {fix.description}: {str(e)}")
                        report.fixes_skipped.append(fix)
                else:
                    logger.debug(f"Skipped low-confidence fix: {fix.description} (confidence: {fix.confidence})")
                    report.fixes_skipped.append(fix)
...
def _parse_warnings_to_fixes(warnings: List[str], errors: List[str] = None) -> List[Fix]:
    ...
    for warning in warnings:
        ...
        if not code_match:
            ...
            missing_field_match = re.search(r"Recommended field '([^']+)' is missing", warning)
    ...
    for error in errors:
        ...
        missing_field_match = re.search(r"Required field '([^']+)' is missing|Recommended field '([^']+)' is missing", error)
        if missing_field_match:
...
def _create_field_addition_fix(field_name: str, is_required: bool) -> Optional[Fix]:
    ...
    if field_name in array_fields:
        ...
        return Fix(
            ...
            warning_code="MISSING_FIELD" if is_required else "MISSING_RECOMMENDED_FIELD"
        )
...
def _apply_data_movement_fix_to_manifest(manifest: OKHManifest, fix: Fix) -> None:
    ...
    elif source_field == "maintenance_instructions" and target_field in ["operating_instructions", "making_instructions"]:
...
def auto_fix_okw_facility(
    ...
    fixes = _parse_warnings_to_fixes(validation_result.warnings, validation_result.errors)
...
def _apply_case_error_fix_to_facility(facility: ManufacturingFacility, fix: Fix) -> None:
...
def _apply_data_movement_fix_to_facility(facility: ManufacturingFacility, fix: Fix) -> None:
...
def _apply_field_removal_fix_to_facility(facility: ManufacturingFacility, fix: Fix) -> None:
...
def _apply_field_addition_fix_to_facility(facility: ManufacturingFacility, fix: Fix) -> None:
