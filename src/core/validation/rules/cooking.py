...
class CookingValidationRules(BaseValidationRules):
    ...
    def get_validation_rules(self, quality_level: str) -> Dict[str, Any]:
        ...
        if not self.validate_quality_level(quality_level):
            raise ValueError(f"Unsupported quality level '{quality_level}' for cooking domain")
    ...
    def get_required_fields(self, quality_level: str) -> List[str]:
        ...
        if quality_level == "home":
            ...
            return base_fields + [
                'cooking_time', 'servings', 'allergen_info'
            ]
        elif quality_level == "professional":
            return base_fields + [
                'cooking_time', 'servings', 'allergen_info', 
                'nutritional_info', 'food_safety_notes'
            ]
    ...
    def get_optional_fields(self, quality_level: str) -> List[str]:
        ...
        base_optional = [
            'description', 'prep_time', 'difficulty_level', 'cuisine_type',
            'dietary_restrictions', 'equipment_needed', 'tips', 'variations'
        ]
        
        if quality_level == "home":
            return base_optional + [
                'cooking_time', 'servings', 'allergen_info', 'nutritional_info'
            ]
        elif quality_level == "commercial":
            return base_optional + [
                'nutritional_info', 'food_safety_notes', 'cost_analysis',
                'storage_instructions', 'serving_suggestions'
            ]
        elif quality_level == "professional":
            return base_optional + [
                'cost_analysis', 'storage_instructions', 'serving_suggestions',
                'quality_standards', 'certifications', 'supplier_info'
            ]
