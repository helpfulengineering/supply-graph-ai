...
def validate_okh_manifest(
    ...
    try:
        ...
        detected_domain = (
            domain
            or okh_manifest.domain
            or _detect_okh_domain_from_manifest(okh_manifest)
        )
...
def validate_okw_facility(
    ...
    try:
        ...
        try:
            ...
            result.add_error(
                f"Failed to parse content as ManufacturingFacility: {str(e)}"
            )
            result.details["parsed_successfully"] = False
            result.details["parse_error"] = str(e)
            return result
...
def _validate_okh_quality_level(
    ...
    quality_requirements = {
        "hobby": {
            ...
            "recommended": (
                ["description", "manufacturing_processes"]
                if domain == "manufacturing"
                else ["description", "tool_list"]
            ),
        },
        "professional": {
            "required": [
            ...
            "recommended": (
                ["description", "manufacturing_specs", "materials", "parts"]
                if domain == "manufacturing"
    ...
    requirements = quality_requirements.get(
        mapped_level, quality_requirements["professional"]
    )
    ...
    if domain == "cooking":
        ...
        if "license" in requirements["required"]:
            ...
            if isinstance(license_data, dict):
                has_any_license = any(
                    license_data.get(k)
                    for k in ["hardware", "documentation", "software"]
                )
                if not has_any_license:
                    result.add_warning(
                        "No license information provided (recommended for cooking recipes)"
                    )
    ...
    for field_name in requirements["required"]:
        ...
        if field_value is None or field_value == [] or field_value == "":
            result.add_error(
                f"Required field '{field_name}' is missing for {quality_level} quality level ({domain} domain)"
            )
    ...
    for field_name in requirements["recommended"]:
        if field_name not in manifest_dict or manifest_dict[field_name] is None:
            result.add_warning(
                f"Recommended field '{field_name}' is missing for {quality_level} quality level ({domain} domain)"
            )
...
def _validate_okw_quality_level(
    ...
    requirements = quality_requirements.get(
        quality_level, quality_requirements["professional"]
    )
    ...
    for field_name in requirements["required"]:
        ...
        if field_name in [
            ...
            if field_value is None:
                result.add_error(
                    f"Required field '{field_name}' is missing for {quality_level} quality level"
                )
        else:
    ...
    for field_name in requirements["recommended"]:
        if field_name not in facility_dict or facility_dict[field_name] is None:
            result.add_warning(
                f"Recommended field '{field_name}' is missing for {quality_level} quality level"
            )
...
def _calculate_okh_completeness(
    manifest: OKHManifest, quality_level: str, domain: str = "manufacturing"
) -> float:
    ...
    if domain == "cooking":
        ...
        quality_fields = {
            "hobby": base_fields + ["description"],
            "professional": base_fields
            + ["description", "manufacturing_processes", "manufacturing_specs"],
            "medical": base_fields
            + [
                "description",
                "manufacturing_processes",
...
def _calculate_okw_completeness(
    facility: ManufacturingFacility, quality_level: str
) -> float:
    ...
    quality_fields = {
        ...
        "professional": base_fields
        + ["equipment", "manufacturing_processes", "typical_materials"],
        "medical": base_fields
        + [
            "equipment",
            "manufacturing_processes",
...
def _generate_okh_suggestions(
    ...
    if not manifest_dict.get("description"):
        result.add_suggestion(
            "Consider adding a description to help users understand the project"
        )

    if domain == "cooking":
        if not manifest_dict.get("tool_list"):
            result.add_suggestion(
                "Consider adding tool_list to specify required kitchen equipment"
            )
        if not manifest_dict.get("materials"):
            result.add_suggestion(
                "Consider adding materials to specify required ingredients"
            )
        if not manifest_dict.get("making_instructions"):
            result.add_suggestion(
                "Consider adding making_instructions with step-by-step recipe instructions"
            )
    else:
        if not manifest_dict.get("manufacturing_processes"):
            result.add_suggestion(
                "Consider adding manufacturing_processes to enable better facility matching"
            )

        if quality_level in ["professional", "medical"] and not manifest_dict.get(
            "manufacturing_specs"
        ):
            result.add_suggestion(
                "Consider adding manufacturing_specs for detailed process requirements"
            )

        if quality_level == "medical" and not manifest_dict.get("standards_used"):
            result.add_suggestion(
                "Consider adding standards_used for medical device compliance"
            )
...
def _validate_okh_metadata(manifest: OKHManifest, result: ValidationResult) -> None:
    ...
    if "original" in metadata:
        ...
        if "tool_list_atoms" in original and isinstance(
            original["tool_list_atoms"], list
        ):
...
def _generate_okw_suggestions(
    ...
    if not facility_dict.get("description"):
        result.add_suggestion(
            "Consider adding a description to help users understand the facility"
        )

    if not facility_dict.get("equipment"):
        result.add_suggestion(
            "Consider adding equipment information for better matching"
        )

    if quality_level in ["professional", "medical"] and not facility_dict.get(
        "certifications"
    ):
        result.add_suggestion(
            "Consider adding certifications to demonstrate quality standards"
        )

    if not facility_dict.get("contact"):
        result.add_suggestion(
            "Consider adding contact information for facility inquiries"
        )
