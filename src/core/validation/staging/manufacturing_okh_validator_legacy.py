...
class OKHValidator(BaseValidator):
    ...
    def validate_supply_tree(self, supply_tree: SupplyTree, 
        ...
        if not supply_tree.validate_snapshot():
            results["valid"] = False
            results["issues"].append("Supply tree validation failed - invalid snapshot references")
        ...
        for req in process_reqs:
            if not self._is_requirement_satisfied(req, supply_tree):
                if getattr(req, 'is_required', True):
                    results["valid"] = False
                    results["issues"].append(f"Required process not satisfied: {req.process_name}")
                else:
                    results["warnings"].append(f"Optional process not satisfied: {req.process_name}")
    ...
    def _validate_tsdc(self, okh_manifest: OKHManifest, tsdc: str) -> bool:
        ...
        if tsdc == "PCB":
            ...
            for part in okh_manifest.parts:
                if "PCB" in part.tsdc:
                    if not any(hasattr(part, field.replace("-", "_")) for field in pcb_fields):
                        return False
            ...
            for part in okh_manifest.parts:
                if "3DP" in part.tsdc:
                    has_3dp_part = True
                    if not (hasattr(part, 'manufacturing_params') and 
                           any(field.replace("-", "_") in part.manufacturing_params for field in tdp_fields)):
                        return False
    ...
    def _calculate_completeness(self, okh_manifest: OKHManifest) -> float:
        ...
        basic_fields = ["title", "repo", "version", "license", "licensor", 
                       "documentation_language", "function", "description"]
        present_count = sum(1 for field in basic_fields if getattr(okh_manifest, field, None))
        scores["basic_info"] = present_count / len(basic_fields)
        
        # Calculate documentation score
        doc_fields = ["manufacturing_files", "design_files", "making_instructions", 
                     "operating_instructions", "bom"]
        ...
        manuf_fields = ["manufacturing_processes", "materials", "manufacturing_specs", "tool_list"]
        manuf_count = sum(1 for field in manuf_fields if getattr(okh_manifest, field, None))
        ...
        elif hasattr(okh_manifest, 'cpc_patent_class') and okh_manifest.cpc_patent_class:
    ...
    def _check_common_issues(self, okh_manifest: OKHManifest, results: Dict):
        ...
        for field in ["manufacturing_files", "design_files", "making_instructions", "operating_instructions"]:
            if hasattr(okh_manifest, field):
                file_list = getattr(okh_manifest, field, [])
                for doc in file_list:
                    if hasattr(doc, 'validate') and not doc.validate():
                        results["warnings"].append(f"Invalid file reference: {doc.path} in {field}")
    ...
    def _is_requirement_satisfied(self, requirement, supply_tree: SupplyTree) -> bool:
        ...
        for workflow in supply_tree.workflows.values():
            for node_id in workflow.graph.nodes:
                ...
                if hasattr(requirement, 'process_name') and requirement.process_name in node.name:
    ...
    def _calculate_confidence(self, supply_tree: SupplyTree, 
                           okh_manifest: OKHManifest) -> float:
        """Calculate confidence score for supply tree"""
        # Count satisfied requirements
        process_reqs = okh_manifest.extract_requirements()
        satisfied_count = sum(1 for req in process_reqs 
