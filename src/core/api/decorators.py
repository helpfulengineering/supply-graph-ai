    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                rate_limit_info = (
                    getattr(request.state, "rate_limit_info", None) if request else None
                )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            is_allowed, rate_limit_info = rate_limit_service.check_rate_limit(
                identifier=identifier, requests_per_minute=requests_per_minute
            )
...
def llm_endpoint(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            llm_params = {
                "use_llm": (
                    request.query_params.get("use_llm", "false").lower() == "true"
                    if request
                    else False
                ),
                "provider": (
                    request.query_params.get("llm_provider", default_provider)
                    if request
                    else default_provider
                ),
                "model": (
                    request.query_params.get("llm_model", default_model)
                    if request
                    else default_model
                ),
                "temperature": (
                    request.query_params.get("llm_temperature") if request else None
                ),
                "max_tokens": (
                    request.query_params.get("llm_max_tokens") if request else None
                ),
            }
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                logger.error(
                    f"Error in paginated_response decorator: {e}", exc_info=True
                )
                raise
            ...
            if items is not None:
                ...
                for item in paginated_items:
                    try:
                        ...
                        logger.error(
                            f"Error converting item to dict: {e}", exc_info=True
                        )
