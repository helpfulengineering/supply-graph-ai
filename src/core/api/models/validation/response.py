...
class ValidationIssue(BaseModel):
    ...
    path: List[str] = Field(
        default_factory=list, description="Path to the field with the issue"
    )
    code: Optional[str] = Field(
        None, description="Issue code for programmatic handling"
    )
...
class ValidationResponse(BaseModel):
    ...
    normalized_content: Dict[str, Any] = Field(
        ..., description="Normalized content after validation"
    )
    issues: Optional[List[ValidationIssue]] = Field(
        None, description="List of validation issues"
    )
    context: Optional[ValidationContextModel] = Field(
        None, description="Validation context used"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "metadata": {"completeness_score": 0.75, "validation_time_ms": 45},
            }
        }
    )
...
class ValidationContextResponse(BaseModel):
    ...
    contexts: List[ValidationContextModel] = Field(
        ..., description="List of available validation contexts"
    )
