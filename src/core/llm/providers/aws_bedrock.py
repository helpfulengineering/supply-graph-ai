...
class AWSBedrockProviderConfig(LLMProviderConfig):
    ...
    model_id: str = ""  # Full model ID (e.g., "anthropic.claude-3-5-sonnet-20241022-v2:0")
    ...
    def __post_init__(self):
        ...
        if not self.use_api_key and not (self.aws_access_key_id and self.aws_secret_access_key):
...
class AWSBedrockProvider(BaseLLMProvider):
    ...
    def __init__(self, config: LLMProviderConfig):
        ...
        if not BOTO3_AVAILABLE:
            raise ImportError("boto3 is required for AWS Bedrock provider. Install it with: pip install boto3")
        ...
        if config.provider_type != LLMProviderType.AWS_BEDROCK:
            raise ValueError(f"Provider type must be AWS_BEDROCK, got {config.provider_type}")
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            if not self._bedrock_config.use_api_key:
                if self._bedrock_config.aws_access_key_id and self._bedrock_config.aws_secret_access_key:
                    client_kwargs["aws_access_key_id"] = self._bedrock_config.aws_access_key_id
                    client_kwargs["aws_secret_access_key"] = self._bedrock_config.aws_secret_access_key
                    if self._bedrock_config.aws_session_token:
                        client_kwargs["aws_session_token"] = self._bedrock_config.aws_session_token
            ...
            self._logger.info(f"Connected to AWS Bedrock in region {self._bedrock_config.region} with model {self._bedrock_config.model_id}")
        ...
        try:
            ...
            response = await loop.run_in_executor(
                ...
                lambda: self._bedrock_client.converse(
                    modelId=self._bedrock_config.model_id,
                    **bedrock_request
                )
            )
    ...
    def _process_bedrock_response(self, response: Dict[str, Any], request: LLMRequest, start_time: datetime) -> LLMResponse:
