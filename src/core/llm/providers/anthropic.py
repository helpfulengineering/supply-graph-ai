...
class AnthropicProvider(BaseLLMProvider):
    ...
    MODEL_PRICING = {
        "claude-sonnet-4-5-20250929": {
            "input": 3.0,
            "output": 15.0,
        },  # $3/$15 per 1M tokens (current default)
        "claude-3-5-sonnet-latest": {
            "input": 3.0,
            "output": 15.0,
        },  # $3/$15 per 1M tokens (deprecated, kept for compatibility)
    ...
    def __init__(self, config: LLMProviderConfig):
        ...
        if config.provider_type != LLMProviderType.ANTHROPIC:
            raise ValueError(
                f"Provider type must be ANTHROPIC, got {config.provider_type}"
            )
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            self._logger.info(
                f"Connected to Anthropic API with model {self.config.model}"
            )
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            llm_response = self._process_anthropic_response(
