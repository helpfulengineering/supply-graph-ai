...
class GoogleVertexAIProvider(BaseLLMProvider):
    ...
    def __init__(self, config: LLMProviderConfig):
        ...
        if config.provider_type != LLMProviderType.GOOGLE:
            raise ValueError(
                f"Provider type must be GOOGLE, got {config.provider_type}"
            )
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            llm_response = self._process_vertex_response(
    ...
    async def _get_credentials(self):
        ...
        if self._vertex_config.service_account_json:
            if os.path.exists(self._vertex_config.service_account_json):
                ...
                raise ValueError(
                    f"Service account file not found: {self._vertex_config.service_account_json}"
                )
    ...
    def _process_vertex_response(
        ...
        metadata = LLMResponseMetadata(
            ...
            metadata={
                ...
                "safety_ratings": (
                    candidates[0].get("safetyRatings") if candidates else None
                ),
