...
class AzureOpenAIProvider(BaseLLMProvider):
    ...
    def __init__(self, config: LLMProviderConfig):
        ...
        if config.provider_type != LLMProviderType.AZURE_OPENAI:
            raise ValueError(f"Provider type must be AZURE_OPENAI, got {config.provider_type}")
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            self._logger.info(f"Connected to Azure OpenAI API with deployment {self._azure_config.deployment_id}")
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            llm_response = self._process_azure_response(response_data, request, start_time)
    ...
    def _process_azure_response(self, response_data: Dict[str, Any], request: LLMRequest, start_time: datetime) -> LLMResponse:
