...
class ProjectExtractor(ABC):
    ...
    def __init__(self, cache_enabled: bool = True, cache_ttl_hours: int = 24):
        ...
        logger.info(f"Initialized {self.platform_name} extractor with cache={'enabled' if cache_enabled else 'disabled'}")
    ...
    def end_extraction(self, success: bool, files_count: int = 0) -> None:
        ...
        if self.metrics:
            ...
            if duration:
                logger.info(f"Extraction completed in {duration.total_seconds():.2f}s - "
