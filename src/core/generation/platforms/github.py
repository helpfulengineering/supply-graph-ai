...
class GitHubExtractor(ProjectExtractor):
        ...
        if cache_dir:
            ...
            if cache_dir_env:
                ...
                try:
                    ...
                    if home_dir.exists():
                        self.cache_dir = (
                            home_dir / ".cache" / "supply-graph-ai" / "github"
                        )
                    else:
                        self.cache_dir = (
    ...
    async def extract_project(self, url: str) -> ProjectData:
        ...
        try:
            ...
            async with httpx.AsyncClient(headers=headers) as client:
                ...
                repo_data = await self._cached_api_request(
                    client, repo_url, repo_cache_key
                )
                ...
                contents_data = (
                    await self._cached_api_request(
                        client, contents_url, contents_cache_key
                    )
                    or []
                )
                ...
                releases_data = (
                    await self._cached_api_request(
                        client, releases_url, releases_cache_key
                    )
                    or []
                )
                ...
                readme_data = await self._cached_api_request(
                    client, readme_url, readme_cache_key
                )
                ...
                for path in license_paths:
                    ...
                    license_data = await self._cached_api_request(
                        client, license_url, license_cache_key
                    )
                    ...
                    if license_data:
                        try:
                            ...
                            license_content = base64.b64decode(
                                license_data["content"]
                            ).decode("utf-8")
                ...
                for bom_path in bom_paths:
                    bom_url = (
                        f"{self.base_url}/repos/{owner}/{repo}/contents/{bom_path}"
                    )
                    bom_cache_key = f"bom_{owner}_{repo}_{bom_path.replace('/', '_')}"
                    bom_data = await self._cached_api_request(
                        client, bom_url, bom_cache_key
                    )
                    ...
                    if bom_data:
                        try:
                            ...
                            bom_content = base64.b64decode(bom_data["content"]).decode(
                                "utf-8"
                            )
                            break
                        except:
                ...
                try:
                    docs_response = await client.get(
                        f"{self.base_url}/repos/{owner}/{repo}/contents/docs?ref=master"
                    )
    ...
    async def _scan_directories_recursively(
        self,
        client,
        owner: str,
        repo: str,
            ...
            if item.get("type") == "file":
                ...
                try:
                    ...
                    dir_response = await client.get(
                        f"{self.base_url}/repos/{owner}/{repo}/contents/{subdir_path}?ref=master"
                    )
