...
class LayerResult:
    ...
    def add_field(self, field_name: str, value: Any, confidence: float, 
        ...
        if not 0.0 <= confidence <= 1.0:
            logger.warning(f"Invalid confidence score {confidence} for field {field_name}, clamping to [0.0, 1.0]")
...
class BaseGenerationLayer(ABC):
    ...
    def log_processing_start(self, project_data: ProjectData):
        ...
        """
        logger.info(f"Starting {self.layer_type.value} layer processing for: {project_data.url}")
    ...
    def log_processing_end(self, result: LayerResult):
        ...
        logger.info(f"Completed {self.layer_type.value} layer processing: {field_count} fields, avg confidence: {avg_confidence:.2f}")
