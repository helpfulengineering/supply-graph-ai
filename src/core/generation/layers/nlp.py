...
class NLPMatcher(BaseGenerationLayer):
    ...
    def _initialize_entity_patterns(self):
        ...
        self.material_patterns = [
            ...
            EntityPattern(
                r"\b(PLA|ABS|PETG|TPU|ASA|PC|Nylon|Resin)\b",
                "material",
                "materials",
                0.9,
            ...
            EntityPattern(
                r"\b(Arduino|Raspberry Pi|ESP32|ESP8266|STM32)\b",
                "component",
                "materials",
                0.8,
                "Microcontroller",
            ...
            EntityPattern(
                r"\b(resistor|capacitor|transistor|diode|LED|sensor)\b",
                "component",
                "materials",
                0.7,
            ...
            EntityPattern(
                r"\b(servo|motor|stepper|DC motor|brushless)\b",
                "component",
                "materials",
                0.8,
                "Motor/actuator",
            ...
            # Hardware Components
            EntityPattern(
                r"\b(screw|bolt|nut|washer|bearing|gear)\b",
                "hardware",
                "materials",
                0.7,
                "Hardware component",
            ...
            EntityPattern(
                r"\b(ball bearing|linear bearing|pulley|belt)\b",
                "hardware",
                "materials",
                0.8,
            ...
            # Materials by Type
            EntityPattern(
                r"\b(aluminum|steel|brass|copper|wood|plastic|acrylic)\b",
                "material",
                "materials",
                0.7,
                "Raw material",
            ...
            EntityPattern(
                r"\b(filament|wire|cable|connector|jack)\b",
                "material",
                "materials",
                0.6,
                "Electrical material",
        ...
        self.process_patterns = [
            ...
            EntityPattern(
                r"\b(3D print|3D printing|3D printed|FDM|SLA|SLS|FFF|fused deposition|selective laser)\b",
                "process",
            ...
            EntityPattern(
                r"\b(CNC|machining|milling|turning|lathe)\b",
                "process",
                "manufacturing_processes",
                0.8,
                "CNC machining",
            ...
            EntityPattern(
                r"\b(solder|soldering|soldered|PCB assembly|circuit assembly)\b",
                "process",
                "manufacturing_processes",
                0.8,
            ...
            EntityPattern(
                r"\b(assemble|assembling|assembled|assembly|mount|mounting|install|installing|attach|attaching)\b",
                "process",
                "manufacturing_processes",
                0.7,
                "Assembly process",
            ...
            EntityPattern(
                r"\b(print|printing|printed)\b",
                "process",
                "manufacturing_processes",
                0.6,
                "Printing (requires context)",
            ),
            EntityPattern(
                r"\b(cut|cutting)\b",
                "process",
                "manufacturing_processes",
                0.6,
                "Cutting (requires context)",
            ),
            EntityPattern(
                r"\b(drill|drilling|bore)\b",
                "process",
                "manufacturing_processes",
                0.6,
                "Drilling (requires context)",
            ),
        ]
        ...
        self.tool_patterns = [
            # 3D Printing Tools
            EntityPattern(
                r"\b(3D printer|printer|extruder|hotend|build plate)\b",
                "tool",
                "tool_list",
                0.9,
            ...
            EntityPattern(
                r"\b(calipers|ruler|measuring|scale|multimeter)\b",
                "tool",
                "tool_list",
                0.8,
                "Measuring tools",
            ...
            # Electronics Tools
            EntityPattern(
                r"\b(soldering iron|solder|flux|desoldering|multimeter)\b",
                "tool",
                "tool_list",
                0.9,
                "Electronics tools",
            ...
            EntityPattern(
                r"\b(oscilloscope|logic analyzer|power supply|breadboard)\b",
                "tool",
                "tool_list",
                0.8,
            ...
            # Mechanical Tools
            EntityPattern(
                r"\b(drill|drill press|CNC|mill|lathe|router)\b",
                "tool",
                "tool_list",
                0.8,
                "Machining tools",
            ...
            # Software Tools
            EntityPattern(
                r"\b(CAD|Fusion 360|SolidWorks|FreeCAD|OpenSCAD)\b",
                "tool",
                "tool_list",
                0.8,
                "CAD software",
            ...
            EntityPattern(
                r"\b(Cura|PrusaSlicer|Slic3r|Simplify3D)\b",
                "tool",
                "tool_list",
                0.8,
                "Slicing software",
    ...
    async def process(self, project_data: ProjectData) -> LayerResult:
        ...
        if readme_content:
            ...
            for field, value in nlp_fields.items():
                confidence = self._calculate_field_confidence(
                    field, value, readme_content
                )
                result.add_field(
                    field, value, confidence, "nlp_analysis", f"README content analysis"
                )
        ...
        for field, value in doc_fields.items():
            if not result.has_field(field):  # Don't override README fields
                ...
                result.add_field(
                    field, value, confidence, "nlp_analysis", "Documentation analysis"
                )
    ...
    def _extract_description(self, doc, content: str) -> Optional[str]:
        ...
        for i, sentence in enumerate(sentences[:5]):  # Check first 5 sentences
            ...
            has_description_indicator = any(
                indicator in sentence.lower() for indicator in description_indicators
            )
            has_skip_indicator = any(
                indicator in sentence.lower() for indicator in skip_indicators
            )
            ...
            if (
                "assembly instructions" in sentence.lower()
                and "is a" in sentence.lower()
            ):
    ...
    def _extract_function(self, doc) -> Optional[str]:
        ...
        exclude_phrases = [
            ...
            "manual",
            "automatic",
            "stage control",  # Too generic, likely from wrong context
        ]
    ...
    def _extract_manufacturing_processes(self, doc, content: str) -> List[str]:
        ...
        for pattern in self.process_patterns:
            ...
            for match in matches:
                ...
                if self._is_manufacturing_context(
                    match, content, manufacturing_context_keywords, doc, pattern
                ):
                    processes.add(matched_text)

        return list(processes)

    def _is_manufacturing_context(
        self, match, content: str, context_keywords: set, doc, pattern
    ) -> bool:
        """
        ...
        has_manufacturing_context = any(
            keyword in context_words for keyword in context_keywords_filtered
        )
        ...
        if doc:
            try:
                ...
                matched_span = doc.char_span(
                    start_pos, end_pos, alignment_mode="expand"
                )
                if matched_span:
                    ...
                    sentence_has_context = any(
                        keyword in sentence_text for keyword in context_keywords
                    )
    ...
    def _calculate_field_confidence(
        self, field: str, value: Any, content: str
    ) -> float:
