...
class FileProcessor:
    ...
    def _initialize_file_type_extensions(self) -> Dict[str, Set[str]]:
        ...
        return {
            ...
            "code": {".py", ".js", ".ts", ".cpp", ".c", ".h", ".hpp", ".java", ".go", ".rs", ".php"},
    ...
    def _initialize_excluded_directories(self) -> Set[str]:
        ...
        return {
            ...
            "node_modules", ".pytest_cache", "venv", "env", ".env"
        }
    ...
    def extract_file_content(self, file_info: FileInfo) -> Optional[str]:
        ...
        if isinstance(file_info.content, str) and len(file_info.content) > 0:
            ...
            printable_ratio = sum(1 for c in file_info.content[:1000] if c.isprintable() or c.isspace()) / min(1000, len(file_info.content))
    ...
    def match_file_patterns(self, files: List[FileInfo]) -> Dict[str, List[Tuple[FileInfo, FilePattern]]]:
        """
    ...
    def filter_excluded_files(self, files: List[FileInfo]) -> List[FileInfo]:
        ...
        for file_info in files:
            ...
            if not any(part.lower() in self._excluded_directories for part in path_parts):
