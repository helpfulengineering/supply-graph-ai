...
class ConfidenceCalculator:
    ...
    def calculate_field_confidence(self, field: str, value: Any, source: str, 
        ...
        if rule and content_quality:
            for quality_factor, adjustment in rule.quality_adjustments.items():
                if self._evaluate_quality_factor(quality_factor, value, content_quality):
    ...
    def _evaluate_quality_factor(self, factor: str, value: Any, content_quality: Dict[str, Any]) -> bool:
        ...
        if factor == 'length_optimal':
            ...
            return isinstance(value, str) and any(term in value.lower() for term in generic_terms)
        elif factor == 'standard_license':
            standard_licenses = ['MIT', 'Apache-2.0', 'GPL-3.0', 'BSD-3-Clause']
            return isinstance(value, str) and any(license in value for license in standard_licenses)
        elif factor == 'semantic_version':
            import re
            return isinstance(value, str) and re.match(r'^\d+\.\d+\.\d+', value) is not None
        elif factor == 'multiple_materials':
            return isinstance(value, list) and len(value) > 1
        elif factor == 'clear_purpose':
            return isinstance(value, str) and len(value) > 20 and 'purpose' in value.lower()
        
        return False
    
    def _apply_value_adjustments(self, confidence: float, field: str, value: Any) -> float:
