...
from ..utils.file_categorization import FileCategorizationResult, FileCategorizationRules
...
class FileCategorizationService:
    ...
    async def categorize_files(
        ...
        for file_info in files:
            ...
            # Skip excluded files
            if file_path in layer1_suggestions and layer1_suggestions[file_path].excluded:
                continue
            ...
            if llm_available:
                try:
                    ...
                    self.logger.warning(f"LLM categorization failed for {file_path}: {e}, falling back to Layer 1")
    ...
    async def _categorize_with_llm(
        ...
        try:
            ...
            # Parse LLM response
            categorization = self._parse_llm_response(
                response.content,
                file_info.path
            )
            ...
            self.logger.error(
                f"Error in LLM categorization for {file_info.path}: {e}",
                exc_info=True
            )
            return None
    ...
    def _parse_llm_response(
        ...
        try:
            ...
            if not json_str:
                self.logger.warning(
                    f"No JSON found in LLM response for {file_path}"
                )
                return None
            ...
            self.logger.error(
                f"Error parsing LLM response for {file_path}: {e}",
                exc_info=True
            )
            return None
