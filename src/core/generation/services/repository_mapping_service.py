...
class RepositoryMappingService:
    ...
    async def update_routing_table(
        ...
        for file_path, categorization in categorizations.items():
            ...
            if existing_route is None or categorization.confidence > existing_route.confidence:
                routing_table.add_route(
        ...
        self.logger.debug(
            f"Updated routing table with {updated_count} categorizations"
        )
    ...
    def _generate_destination_path(
        ...
        destination_dir = type_to_dir.get(
            documentation_type,
            "other"
        )
