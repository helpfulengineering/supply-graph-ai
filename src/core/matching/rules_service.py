...
class RulesService:
    ...
    async def create_rule(self, rule_data: Dict[str, Any]) -> CapabilityRule:
        ...
        if existing_rule:
            raise ValueError(
                f"Rule '{rule.id}' already exists in domain '{rule.domain}'"
            )
        ...
        if not rule_set:
            ...
            rule_set = CapabilityRuleSet(
                domain=rule.domain, version="1.0.0", rules={rule.id: rule}
            )
    ...
    async def update_rule(
        self, domain: str, rule_id: str, rule_data: Dict[str, Any]
    ) -> CapabilityRule:
        """
    ...
    async def import_rule_set(
        self, rule_set: CapabilityRuleSet, partial_update: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    async def reload_rules(self, domain: Optional[str] = None) -> Dict[str, Any]:
        ...
        if domain:
            ...
            if rule_set:
                ...
                return {"reloaded_domains": [], "total_rules": 0}
        else:
            # Reload all domains
            before_count = sum(
                len(rs.rules) for rs in self.rule_manager.rule_sets.values()
            )
    ...
    async def _validate_all_rules(self) -> Dict[str, Any]:
        ...
        for domain, rule_set in self.rule_manager.rule_sets.items():
            ...
            validation_result = await self.validation_service.validate_rule_set(
                rule_set_dict
            )
