...
class CapabilityRule:
    ...
    def __post_init__(self):
        ...
        if not self.capability or not self.capability.strip():
            raise ValueError(
                f"Rule {self.id}: capability cannot be empty or whitespace"
            )
        ...
        for i, req in enumerate(self.satisfies_requirements):
            ...
            if not req.strip():
                raise ValueError(
                    f"Rule {self.id}: requirement at index {i} cannot be empty or whitespace"
                )

        # Validate confidence
        if not 0.0 <= self.confidence <= 1.0:
            raise ValueError(
                f"Rule {self.id}: confidence must be between 0.0 and 1.0, got {self.confidence}"
            )
        ...
        if len(self.satisfies_requirements) != len(
            set(r.lower().strip() for r in self.satisfies_requirements)
        ):
            logger.warning(
                f"Rule {self.id}: duplicate requirements detected (case-insensitive)"
            )
    ...
    def requirement_can_be_satisfied_by(
...
class CapabilityRuleSet:
    ...
    def __post_init__(self):
        ...
        if not self.rules:
            raise ValueError(
                f"Rule set for domain '{self.domain}' must contain at least one rule"
            )
        ...
        if not re.match(r"^\d+\.\d+\.\d+$", self.version):
            logger.warning(
                f"Rule set version '{self.version}' does not follow semantic versioning (X.Y.Z)"
            )
    ...
    def add_rule(self, rule: CapabilityRule) -> None:
        ...
        if rule.domain != self.domain and rule.domain != "general":
            logger.warning(
                f"Adding rule {rule.id} with domain {rule.domain} to rule set for domain {self.domain}"
            )
    ...
    def find_rules_for_capability_requirement(
    ...
    def to_dict(self, include_metadata: bool = False) -> Dict[str, Any]:
        ...
        result = {
            ...
            "description": self.description,
            "rules": {
                rule_id: rule.to_dict(include_metadata=include_metadata)
                for rule_id, rule in self.rules.items()
            },
        ...
        try:
            ...
            for rule_id, rule_data in data.get("rules", {}).items():
                try:
                    ...
                    raise ValueError(
                        f"Invalid rule data for rule '{rule_id}': {str(e)}"
                    ) from e
...
class CapabilityRuleManager:
    ...
    async def initialize(self) -> None:
        ...
        logger.info(
            f"Initializing CapabilityRuleManager with rules directory: {self.rules_directory}"
        )
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.rule_sets)} rule sets: {list(self.rule_sets.keys())}"
            )
        except Exception as e:
            logger.error(
                f"Failed to initialize CapabilityRuleManager: {e}", exc_info=True
            )
            raise
    ...
    async def _load_all_rule_sets(self) -> None:
        ...
        rule_files = (
            list(rules_path.glob("*.yaml"))
            + list(rules_path.glob("*.yml"))
        ...
        for rule_file in rule_files:
            try:
                ...
                logger.error(
                    f"Failed to load rule set from {rule_file}: {e}", exc_info=True
                )
                continue
    ...
    async def _load_rule_set_from_file(self, file_path: Path) -> None:
        ...
        logger.info(
            f"Loaded rule set for domain '{rule_set.domain}' with {len(rule_set.rules)} rules"
        )
    ...
    def add_rule_set(self, rule_set: CapabilityRuleSet) -> None:
        ...
        self.rule_sets[rule_set.domain] = rule_set
        logger.info(
            f"Added rule set for domain '{rule_set.domain}' with {len(rule_set.rules)} rules"
        )
    ...
    def get_rules_by_type(
        self, domain: str, rule_type: RuleType
    ) -> List[CapabilityRule]:
    ...
    def find_rules_for_capability_requirement(
        self, domain: str, capability: str, requirement: str
    ) -> List[CapabilityRule]:
...
class CapabilityMatcher:
    ...
    async def capability_can_satisfy_requirement(
        self, capability: str, requirement: str, domain: str
    ) -> bool:
        """
    ...
    async def requirement_can_be_satisfied_by(
        self, requirement: str, capability: str, domain: str
    ) -> bool:
        """
        Check if a requirement can be satisfied by a capability.
        ...
        return await self.capability_can_satisfy_requirement(
    ...
    async def match_requirements_to_capabilities(
        ...
        for requirement_obj in requirements:
            for capability_obj in capabilities:
                ...
                requirement_value = (
                    requirement_obj.get(requirement_field, "").lower().strip()
                )
                capability_value = (
                    capability_obj.get(capability_field, "").lower().strip()
                )
                ...
                # Find matching rules
                rules = self.rule_manager.find_rules_for_capability_requirement(
                    domain, capability_value, requirement_value
                )
                    ...
                    results.append(
                        CapabilityMatchResult(
                            ...
                            transformation_details=[
                                f"Capability '{capability_value}' can satisfy requirement '{requirement_value}'"
                            ],
                        )
                    )
