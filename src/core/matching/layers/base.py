...
class BaseMatchingLayer(ABC):
    ...
    def __init__(self, layer_type: MatchingLayer, domain: str = "general"):
        ...
        logger.info(
            f"Initialized {layer_type.value} matching layer for domain: {domain}"
        )
    ...
    def start_matching(self, requirements: List[str], capabilities: List[str]) -> None:
        ...
        logger.info(
            f"Starting {self.layer_type.value} matching: {req_count} requirements vs {cap_count} capabilities"
        )
    ...
    def end_matching(self, success: bool, matches_found: int = 0) -> None:
        ...
        if self.metrics:
            ...
            if duration:
                logger.info(
                    f"Matching completed in {duration.total_seconds():.2f}s - "
    ...
    def create_matching_result(
        ...
        **kwargs,
        ...
        if not 0.0 <= confidence <= 1.0:
            logger.warning(
                f"Invalid confidence score {confidence}, clamping to [0.0, 1.0]"
            )
    ...
    def log_matching_start(
        ...
        cap_count = len(capabilities) if capabilities is not None else 0
        logger.info(
            f"[{self.layer_type.value}] Starting matching: {req_count} requirements vs {cap_count} capabilities"
        )
    ...
    def log_matching_end(self, results: List[MatchingResult]) -> None:
        ...
        logger.info(
            f"[{self.layer_type.value}] Matching completed: {matches_found}/{len(results)} matches found"
        )

    def handle_matching_error(
        self, error: Exception, results: List[MatchingResult]
    ) -> List[MatchingResult]:
        """
