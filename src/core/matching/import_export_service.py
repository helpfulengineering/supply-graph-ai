...
class ImportExportService:
            ...
            if domain:
                ...
                for dom, domain_data in data["domains"].items():
                    try:
                        ...
                        results["imported_rules"] += domain_result.get("imported_rules", 0)
                        results["updated_rules"] += domain_result.get("updated_rules", 0)
    ...
    async def _import_single_domain(
        ...
        # Apply changes
        result = await self.rules_service.import_rule_set(rule_set, partial_update=partial_update)
