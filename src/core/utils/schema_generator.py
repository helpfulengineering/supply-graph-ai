...
try:
    ...
    raise ImportError("Pydantic is required for schema generation. Install with: pip install pydantic")


def generate_json_schema(dataclass_type: Type, title: Optional[str] = None) -> Dict[str, Any]:
    """
...
def _generate_schema_manually(dataclass_type: Type, title: Optional[str] = None) -> Dict[str, Any]:
    """
        ...
        if hasattr(field_type, "__args__"):
            ...
            if origin is type(None) or (hasattr(origin, "__name__") and origin.__name__ == "Union"):
                ...
                if len(non_none_types) == 1:
                    ...
                    return {
                        "anyOf": [_type_to_schema(t) for t in non_none_types]
                    }
...
def _type_to_schema(python_type: Type) -> Dict[str, Any]:
    ...
    if not inspect.isclass(python_type) and callable(python_type) and python_type.__name__ in ['any', 'all']:
