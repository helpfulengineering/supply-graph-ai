...
class SecretsManager:
    ...
    def _init_gcp(self):
        ...
        try:
            ...
            if not project_id:
                raise ValueError("GOOGLE_CLOUD_PROJECT or GCP_PROJECT_ID must be set for GCP Secret Manager")
            self._client = secretmanager.SecretManagerServiceClient()
            self._project_id = project_id
            logger.info(f"Initialized GCP Secret Manager client for project {project_id}")
        except ImportError:
    ...
    def get_secret(self, secret_name: str, default: Optional[str] = None) -> Optional[str]:
        ...
        if self.provider != SecretsProvider.ENV and self._initialized:
            try:
                ...
                logger.debug(f"Failed to get secret {secret_name} from {self.provider.value}: {e}")
