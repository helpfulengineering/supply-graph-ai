...
class ContentValidator:
    ...
    def _validate_okw_content(self, data: dict) -> bool:
        ...
        try:
            ...
            optional_fields = ['manufacturing_processes', 'equipment', 'location', 'name']
    ...
    async def discover_files(self, file_type: str) -> List[FileInfo]:
        ...
        for strategy in self.discovery_strategies:
            try:
                ...
                if files:
                    logger.info(f"Found {len(files)} {file_type} files using {strategy.__name__}")
                    return files
            except Exception as e:
    ...
    async def _discover_by_metadata(self, file_type: str) -> List[FileInfo]:
        ...
        try:
            async for obj in self.storage_manager.list_objects():
                ...
                if (metadata.get("file-type") == file_type or 
                    metadata.get("domain") == file_type or
                    metadata.get("type") == file_type):
