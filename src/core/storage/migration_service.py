...
class StorageMigrationService:
    ...
    def generate_organized_path(self, file_type: str, file_data: Dict[str, Any]) -> str:
        ...
        if file_type == "okh":
            ...
            file_id = file_data.get("id", "unknown")
            if not file_id or file_id == "unknown":
                file_id = self.uuid_validator.generate_uuid_from_string(
                    str(datetime.now())
                )
    ...
    async def migrate_files(self, dry_run: bool = True) -> MigrationReport:
        ...
        try:
            ...
            logger.info(
                f"Migration completed: {report.files_migrated} migrated, "
            ...
            if not dry_run:
                ...
                await self.storage_manager.put_object(
                    ...
                    metadata={
                        ...
                        "file_type": file_info.file_type,
                        "data_quality_fixes": (
                            json.dumps(data_quality_fixes)
                            if data_quality_fixes
                            else "[]"
                        ),
                    },
                )
    ...
    async def rollback_migration(
        ...
        try:
            ...
            if migration_id:
                ...
                migrations_to_rollback = (
                    [self.migration_log[-1]] if self.migration_log else []
                )
                ...
                try:
                    ...
                    try:
                        ...
                        content_to_restore = (
                            original_content
                            if original_content
                            else migrated_file.decode("utf-8")
                        )
                    except Exception as e:
                        logger.warning(
                            f"File not found at new location {new_path}: {e}"
                        )
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to delete migrated file at {new_path}: {e}"
                            )
            ...
            if not success and files_restored == 0:
                ...
                result["message"] = (
                    f"Rollback partially successful - {files_restored} restored, {files_failed} failed"
                )
            else:
                result["message"] = (
                    f"Rollback successful - {files_restored} file(s) restored"
                )

            logger.info(
                f"Rollback completed: {files_restored} restored, {files_failed} failed"
            )
            return result
