...
class GCSProvider(StorageProvider):
        ...
        if not self._connected:
            try:
                ...
                if not bucket_exists:
                    logger.warning(f"Bucket {self.config.bucket_name} does not exist, but continuing...")
                    # Don't create bucket automatically - user should create it first
                
                self._connected = True
                logger.info(f"Connected to Google Cloud Storage: {self.config.bucket_name}")
    ...
    async def set_bucket_policy(self, bucket_name: str, policy: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.warning("Bucket policy setting is simplified for GCS. Use IAM policies for full control.")
            return True
        except Exception as e:
