...
class BaseKnowledgeProvider(ABC):
    ...
    def get_substitutions(
        self, requirement: Requirement, capabilities: List[Capability]
    ) -> List[Substitution]:
        """Find possible substitutions for a requirement"""
        pass

    @abstractmethod
    def validate_match(self, requirement: Requirement, capability: Capability) -> float:
...
class KnowledgeBase(ABC):
    ...
    @abstractmethod
    def add_rule(
        self, rule_type: str, rule_data: Dict[str, Any], confidence: float = 1.0
    ):
    ...
    @abstractmethod
    def query_rules(
        self, rule_type: str, query: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
...
class FeedbackCollector(ABC):
    ...
    @abstractmethod
    def record_feedback(
        self, match_result: MatchResult, success: bool, feedback_data: Dict[str, Any]
    ):
