...
class SupplyTreeSolution:
    ...
    def __hash__(self):
        """Enable Set operations by hashing on facility_name, okh_reference, and okw_reference"""
        return hash(
            (self.tree.facility_name, self.tree.okh_reference, self.tree.okw_reference)
        )
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, SupplyTreeSolution):
            return False
        return (
            self.tree.facility_name,
            self.tree.okh_reference,
            self.tree.okw_reference,
        ) == (
            other.tree.facility_name,
            other.tree.okh_reference,
            other.tree.okw_reference,
        )
