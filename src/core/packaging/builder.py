...
class PackageBuilder:
    ...
    async def build_package(
        ...
        logger.info(f"Package built successfully: {len(file_inventory)} files, {metadata.total_size_bytes} bytes")
        return metadata
    ...
    async def _download_parts_files(self, parts: List[PartSpec], package_path: Path, repo_url: str = None) -> List[FileInfo]:
        ...
        for part in parts:
            ...
            if part.source:
                source_list = part.source if isinstance(part.source, list) else [part.source]
                source_docs = [DocumentRef(title=f"source_{i}", path=path, type=DocumentationType.DESIGN_FILES) 
            ...
            if part.export:
                export_list = part.export if isinstance(part.export, list) else [part.export]
                export_docs = [DocumentRef(title=f"export_{i}", path=path, type=DocumentationType.DESIGN_FILES) 
            ...
            if part.auxiliary:
                aux_list = part.auxiliary if isinstance(part.auxiliary, list) else [part.auxiliary]
                aux_docs = [DocumentRef(title=f"auxiliary_{i}", path=path, type=DocumentationType.MANUFACTURING_FILES) 
    ...
    def _categorize_documents_by_type(self, documents: List[DocumentRef]) -> Dict[str, List[DocumentRef]]:
    ...
    def _discover_files_to_download(self, manifest: OKHManifest, options: BuildOptions) -> Dict[str, List[Dict[str, Any]]]:
        """
    ...
    async def _save_package_metadata(self, metadata: PackageMetadata, package_path: Path):
