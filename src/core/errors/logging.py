...
class LLMLogger:
    ...
    def log_llm_request(
        ...
        self.logger.info(
            f"LLM request to {provider}/{model} for {operation}", extra=log_data
        )
    ...
    def log_llm_response(
        ...
        self.logger.info(
            f"LLM response from {provider}/{model} in {duration_ms:.2f}ms",
            extra=log_data,
        )
    ...
    def log_operation(
        ...
        **metadata,
        ...
        if duration_ms > 5000:  # > 5 seconds
            self.logger.warning(
                f"Slow operation {operation}: {duration_ms:.2f}ms", extra=log_data
            )
        elif duration_ms > 1000:  # > 1 second
            self.logger.info(
                f"Operation {operation}: {duration_ms:.2f}ms", extra=log_data
            )
        else:
            self.logger.debug(
                f"Operation {operation}: {duration_ms:.2f}ms", extra=log_data
...
class AuditLogger:
    ...
    def log_user_action(
        ...
        self.logger.info(
            f"User {user_id} performed {action} on {resource}: {result}", extra=log_data
        )
    ...
    def log_security_event(
        ...
        self.logger.warning(
            f"Security event: {event_type} ({severity})", extra=log_data
        )
