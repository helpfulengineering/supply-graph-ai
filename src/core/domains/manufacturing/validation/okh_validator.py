...
class ManufacturingOKHValidator(Validator):
    ...
    async def _validate_okh_manifest(
        self, okh_manifest: OKHManifest, context: Optional[ValidationContext] = None
    ) -> ValidationResult:
        ...
        await self._validate_manufacturing_processes(
            okh_manifest, quality_level, result
        )
    ...
    async def _validate_tsdc_compliance(
        self, okh_manifest: OKHManifest, quality_level: str, result: ValidationResult
    ):
        ...
        for part in getattr(okh_manifest, "parts", []):
            if hasattr(part, "tsdc") and "3DP" in part.tsdc:
                ...
                if hasattr(part, "manufacturing_params"):
                    ...
                    if missing_3dp_fields and quality_level in [
                        "professional",
                        "medical",
                    ]:
                        result.add_error(
        ...
        for part in getattr(okh_manifest, "parts", []):
            if hasattr(part, "tsdc") and "MECH" in part.tsdc:
                ...
                if hasattr(part, "manufacturing_params"):
                    ...
                    if missing_mech_fields and quality_level in [
                        "professional",
                        "medical",
                    ]:
                        result.add_error(
    ...
    def _calculate_completeness(
        self, okh_manifest: OKHManifest, quality_level: str
    ) -> float:
