...
class MfgDirectMatcher(DirectMatcher):
    ...
    def __init__(self, near_miss_threshold: int = 2):
        ...
        """
        super().__init__(
            domain="manufacturing", near_miss_threshold=near_miss_threshold
        )
    ...
    def get_domain_specific_confidence_adjustments(
        ...
        if self._contains_material_keywords(
            req_lower
        ) and self._contains_material_keywords(cap_lower):
            adjustment += 0.05

        # Component matching gets slight boost
        if self._contains_component_keywords(
            req_lower
        ) and self._contains_component_keywords(cap_lower):
            adjustment += 0.05

        # Tool matching gets slight boost
        if self._contains_tool_keywords(req_lower) and self._contains_tool_keywords(
            cap_lower
        ):
            adjustment += 0.05

        # Process matching gets slight boost
        if self._contains_process_keywords(
            req_lower
        ) and self._contains_process_keywords(cap_lower):
            adjustment += 0.05

        # Specification matching gets slight boost
        if self._contains_specification_keywords(
            req_lower
        ) and self._contains_specification_keywords(cap_lower):
            adjustment += 0.03

        # Penalty for obvious mismatches (e.g., material vs tool)
        if (
            self._contains_material_keywords(req_lower)
            and self._contains_tool_keywords(cap_lower)
        ) or (
            self._contains_tool_keywords(req_lower)
            and self._contains_material_keywords(cap_lower)
        ):
            adjustment -= 0.1
    ...
    def match_materials(
        self, required_materials: List[str], available_materials: List[str]
    ) -> List[MatchingResult]:
        """
    ...
    def match_components(
        self, required_components: List[str], available_components: List[str]
    ) -> List[MatchingResult]:
        """
    ...
    def match_tools(
        self, required_tools: List[str], available_tools: List[str]
    ) -> List[MatchingResult]:
        """
    ...
    async def match_processes(
        self, required_processes: List[str], available_processes: List[str]
    ) -> List[MatchingResult]:
        """
    ...
    def match_okh_requirements(
        self, okh_data: Dict[str, Any], okw_capabilities: Dict[str, Any]
        ...
        """
        results = {"materials": [], "components": [], "tools": [], "processes": []}
        ...
        if required_materials and available_materials:
            results["materials"] = self.match_materials(
                required_materials, available_materials
            )

        if required_components and available_components:
            results["components"] = self.match_components(
                required_components, available_components
            )
        ...
        if required_processes and available_processes:
            results["processes"] = self.match_processes(
                required_processes, available_processes
            )
