...
class CookingDirectMatcher(DirectMatcher):
    ...
    def get_domain_specific_confidence_adjustments(
        ...
        if self._contains_ingredient_keywords(
            req_lower
        ) and self._contains_ingredient_keywords(cap_lower):
            adjustment += 0.05

        # Equipment matching gets slight boost
        if self._contains_equipment_keywords(
            req_lower
        ) and self._contains_equipment_keywords(cap_lower):
            adjustment += 0.05

        # Technique matching gets slight boost
        if self._contains_technique_keywords(
            req_lower
        ) and self._contains_technique_keywords(cap_lower):
            adjustment += 0.05

        # Measurement units matching gets slight boost
        if self._contains_measurement_units(
            req_lower
        ) and self._contains_measurement_units(cap_lower):
            adjustment += 0.03

        # Penalty for obvious mismatches (e.g., ingredient vs equipment)
        if (
            self._contains_ingredient_keywords(req_lower)
            and self._contains_equipment_keywords(cap_lower)
        ) or (
            self._contains_equipment_keywords(req_lower)
            and self._contains_ingredient_keywords(cap_lower)
        ):
            adjustment -= 0.1
    ...
    def match_ingredients(
        self, required_ingredients: List[str], available_ingredients: List[str]
    ) -> List[MatchingResult]:
        """
    ...
    def match_equipment(
        self, required_equipment: List[str], available_equipment: List[str]
    ) -> List[MatchingResult]:
        """
    ...
    def match_techniques(
        self, required_techniques: List[str], available_techniques: List[str]
    ) -> List[MatchingResult]:
        """
    ...
    def match_recipe_requirements(
        self, recipe_data: Dict[str, Any], kitchen_capabilities: Dict[str, Any]
        ...
        """
        results = {"ingredients": [], "equipment": [], "techniques": []}
