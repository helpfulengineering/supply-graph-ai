...
class ConfigurationValidator:
        ...
        for component, component_result in result.details.items():
            if isinstance(component_result, dict):
                if not component_result.get("valid", True):
                    result.add_error(
                        f"{component}: {component_result.get('error', 'Validation failed')}"
                    )
    ...
    def _validate_config_files(self) -> Dict[str, Any]:
        ...
        config_files = ["config/llm_config.json", ".env", "config/domains.yaml"]
            ...
            if provider_config.api_base_url:
                if not provider_config.api_base_url.startswith(("http://", "https://")):
                    result["warnings"].append(
                        f"Invalid base URL format for {provider_name}"
                    )

            # Provider-specific validations
            if provider == LLMProvider.OPENAI:
                if not provider_config.organization_id and not os.getenv(
