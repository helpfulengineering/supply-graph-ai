...
│   │   │   ├── __init__.py
│   │   │   ├── app_secrets.py
│   │   │   │       def shared_secret_env_refs()
│   │   │   │       def mirrored_secret_names()
│   │   │   │       def mirrored_secret_env_vars()
│   │   │   ├── config.py
...
│   │   │       def _layer_finding()
│   │   ├── probe_async_generation.py
│   │   │       class ProbeAsyncGenerationLoop (_options, discover, observe...)
│   │   ├── probe_cache.py
...
│   ├── setup_storage.py
│   ├── validate_docs.py
│   │       class DocValidator (__init__, check_auth_header, _doc_files...)
│   │       def main()
...
│   │   ├── security_policy.py
│   │   │       class SecurityMode
│   │   │       class SecurityPolicy (to_public_dict)
│   │   │       def parse_security_mode()
│   │   │       def _peacetime_requires_auth_for_writes()
│   │   │       def get_security_policy()
│   │   │       def allows_full_metadata_logging()
│   │   ├── settings.py
...
│       │   │   │   │       class PackageZipItem
│       │   │   │   │       class PackageDownloadZipRequest (_require_items)
│       │   │   │   └── response.py
...
│       │   │       def parse_node_role()
│       │   ├── okw_catalog.py
│       │   │       class OkwCatalogRecord (record_payload)
│       │   │       class SignedOkwRecord
│       │   │       class OkwCatalogIndex (get_signed_record)
│       │   │       def _sign_okw_record()
│       │   ├── okw_ingest.py
│       │   │       class OkwIngestError
│       │   │       class OkwIngestResult
│       │   │       def verify_signed_okw_record()
│       │   ├── okw_sync.py
│       │   │       class OkwSyncPeerResult
│       │   ├── package_fetch.py
│       │   │       class PackageFetchResult
│       │   ├── package_pointer.py
│       │   │       def _package_search_roots()
│       │   │       def _normalize_hash()
│       │   │       def _find_archive()
│       │   │       def _pin_for_dir()
│       │   │       def _pointer_from_dir()
│       │   │       def find_package_dir_for_manifest()
│       │   │       def resolve_package_pointer()
│       │   │       def find_package_dir_by_bundle_hash()
│       │   │       def package_dir_to_archive_bytes()
│       │   │       def _is_within_dest()
...
│       │   ├── attestation.py
│       │   │       class Attestation (signing_payload)
...
│       │   ├── binding.py
│       │   │       class IdentityBinding (signing_payload)
│       │   │       class DomainBindRequest
│       │   │       class DomainBindStartResponse
│       │   │       class DomainVerifyRequest
│       │   │       class OAuthBindRequest
│       │   │       class DirectoryEntry
│       │   │       class DirectoryPublishRequest
│       │   │       def domain_external_id()
│       │   │       def oauth_external_id()
│       │   │       def well_known_url()
│       │   │       def well_known_document()
│       │   ├── bom.py
...
│       │   ├── disclosure.py
│       │   │       class DisclosureAudience
│       │   │       class DisclosureGroup
│       │   │       class AudienceDisclosure
│       │   │       class DisclosureProfile
│       │   │       class DisclosureBody
│       │   │       class DisclosureResponse
│       │   │       class DisclosurePreviewResponse
│       │   │       def default_disclosure_profile()
...
│       │   ├── space.py
│       │   │       class SpaceClaim (signing_payload)
│       │   │       class SpaceClaimRequest
│       │   ├── supply_trees.py
│       │   │       class ResourceType
│       │   │       class ProcessStatus
...
│       │   │       class AttestationStore (__init__, _key)
│       │   ├── auth_storage.py
│       │   │       class AuthStorage (__init__, _get_storage_key, _get_index_key)
│       │   ├── base.py
│       │   │       class StorageConfig (__init__)
│       │   │       class StorageMetadata (__init__)
│       │   │       class StorageProvider
│       │   ├── binding_store.py
│       │   │       class BindingStore (__init__, _key)
│       │   │       class DirectoryStore (__init__, _key)
│       │   ├── constants.py
│       │   │       def build_solution_key()
│       │   │       def build_solution_metadata_key()
│       │   ├── disclosure_store.py
│       │   │       class DisclosureStore (__init__, _key)
│       │   ├── grant_store.py
...
│       │   ├── space_claim_store.py
│       │   │       class SpaceClaimStore (__init__, _key)
│       │   └── visibility_store.py
│       │           class VisibilityStore (__init__, _key)
│       ├── taxonomy/
│       │   ├── __init__.py
...
└── tests/
    ...
    │           def main()
    ├── api/
    │   ├── test_attestation_routes.py
    │   │       def _get_app()
    │   ├── test_binding_routes.py
    │   │       def _get_app()
    ...
    │   │       def _get_app()
    │   ├── test_okw_spaceapi.py
    │   │       def fixture()
    │   │       def _app()
    │   │       def test_capabilities_land_in_the_key_mom_reads()
    │   │       def test_unrecognized_processes_are_dropped_from_the_tags()
    │   │       def test_no_street_address_or_contact_is_published()
    │   │       def test_country_code_is_alpha2_and_omitted_when_unknown()
    │   │       def test_spaceapi_conformance_is_not_claimed()
    │   │       def test_lifecycle_status_does_not_masquerade_as_realtime_state()
    ...
    │   ├── __init__.py
    │   ├── test_catalog.py
    │   │       def _patch_auth_attestations()
    │   │       def test_manifest_content_hash_is_stable()
    │   ├── test_disclosure.py
    │   │       def test_default_profile_is_identity_only()
    │   │       def test_project_fail_closed_strips_location_and_equipment()
    │   │       def test_project_with_location_group()
    │   │       def test_groups_for_audience_always_includes_identity()
    │   ├── test_discovery.py
    ...
    │   │       def test_verify_signed_record_accepts_valid_provenance()
    │   │       def test_verify_signed_record_rejects_tampered_provenance()
    │   │       def test_verify_signed_record_rejects_bad_attestation()
    │   ├── test_merkle.py
    ...
    │   ├── test_okw_catalog.py
    │   ├── test_okw_ingest_provenance.py
    │   ├── test_package_pointer.py
    │   │       def test_resolve_package_pointer_from_local_package()
    │   │       def test_resolve_package_pointer_absent()
    │   │       def test_manifest_hash_unchanged_by_package_pointer_fields()
    │   │       def test_try_extract_rejects_zip_slip()
    ...
    │   ├── test_list_visibility_field.py
    │   │       def _register()
    │   │       def test_okh_list_reports_visibility_and_hides_it_from_others()
    │   │       def test_okw_list_reports_visibility()
    ...
    │   │       def iter_canonical_ids()
    │   │       def facilities_claiming_process()
    │   │       def minimal_okh_for_process()
    ...
    │   │       def _mom_live_enabled()
    │   └── test_wikidata_qids_live.py
    │           def _wikidata_live_enabled()
    │           def _tokens()
    │           def _qid_processes()
    ...
    │   │       def _fe_routes_from_app_dir()
    │   │       def actual_fe_routes()
    │   │       def actual_fe_api_prefixes()
    │   │       def _scan_sources()
    │   │       def _imports_island_client()
    │   │       def _call_site_patterns()
    │   │       def fe_api_call_sites()
    │   │       def fe_called_api_paths()
    │   │       def fe_calls()
    │   │       def openapi_paths()
    │   │       def schema_d_ts_paths()
    │   │       def layer_diff()
    │   ├── manifest.py
    │   │       class Area
    │   │       class Endpoint
    ...
    │   │       def _decision()
    │   │       def _composed()
    │   │       def uncalled_endpoint_paths()
    │   │       def endpoints_by_status()
    │   │       def doc_areas()
    │   │       def site_doc_paths()
    │   │       def site_doc_for_path()
    ...
    │   ├── test_compose_defaults.py
    │   │       def _compose()
    │   │       def _declared_defaults()
    │   │       def _schema_default()
    │   │       def test_compose_defaults_match_the_schema_unless_declared()
    │   │       def test_the_llm_kill_switch_is_not_inverted()
    │   │       def _resolved_with_env_example()
    ...
    │   └── test_viewer_scope_ratchet.py
    │           def _list_calls_missing_a_viewer()
    │           def test_no_route_lists_records_without_a_viewer_scope()
    ...
    └── unit/
        ...
        │       def service()
        │       def _node_scope()
        ├── test_auth_service_registration.py
        │       class _InMemoryManager (__init__)
        │       class _FakeStorageService (__init__)
        │       def service()
        ├── test_auth_service_spaces.py
        │       class _InMemoryManager (__init__)
        │       class _FakeStorageService (__init__)
        │       def service()
        ...
        │       def test_incomplete_config_is_refused()
        ...
        ├── test_bom_nlp_cost.py
        │       class _PipelineTripwire (__init__, __call__, tokenizer)
        │       class _Token (__init__)
        │       def collector()
        │       def test_bom_likelihood_uses_the_tokenizer_not_the_pipeline()
        │       def test_bom_likelihood_still_scores_on_keywords()
        │       def test_section_finding_does_not_parse_the_document()
        │       def test_no_nlp_available_is_handled()
        ...
        │       class TestDeployEnvVars (test_production_applies_storage_target, test_staging_enables_federation_under_the_names_settings_reads, test_staging_federation_makes_no_outbound_peer_calls...)
        ...
        ├── test_docs_site_build.py
        │       def build_docs_site()
        │       def test_badge_lands_directly_under_the_h1()
        │       def test_badge_text_differs_per_status()
        │       def test_page_without_an_h1_still_gets_a_badge()
        │       def test_unknown_status_leaves_the_page_untouched()
        │       def test_only_the_first_h1_is_matched()
        │       def test_whats_built_declares_itself_generated()
        ...
        │       def _steps()
        │       def _run_text()
        │       def test_worker_deploys_before_the_api()
        │       def test_worker_and_api_deploy_the_same_digest()
        │       def test_worker_deploy_is_gated_like_the_api_deploy()
        │       def test_worker_deploy_verifies_the_rolled_image()
        │       def test_async_generation_probe_is_the_final_gate()
        │       def test_release_deploys_worker_api_and_frontend()
        ...
        ├── test_repo_map_generation.py
        │       def _load_generator()
        │       def _make_repo()
        │       def test_identity_comes_from_the_project_name()
        │       def test_identity_falls_back_to_the_directory_without_a_pyproject()
        │       def test_identity_falls_back_when_the_pyproject_is_unreadable()
        │       def test_identity_falls_back_when_the_project_name_is_missing()
        │       def test_map_does_not_depend_on_the_checkout_directory_name()
        │       def test_map_is_deterministic_across_runs()
        │       def test_map_records_the_symbols_it_found()
        │       def test_parse_failures_are_detected_in_a_generated_map()
        │       def test_a_clean_map_reports_no_parse_failures()
        │       def test_generator_refuses_to_write_a_map_with_missing_symbols()
        │       def _run_check()
        │       def test_check_passes_when_the_map_is_current()
        │       def test_check_fails_when_the_map_has_drifted()
        │       def test_check_fails_when_the_map_is_absent()
        ...
        │       def _sol()
        ├── test_safe_paths.py
        │       def test_legitimate_package_paths_still_work()
        │       def test_escaping_paths_are_rejected()
        │       def test_a_symlinked_base_is_judged_on_where_it_lands()
        │       def test_object_keys_stay_under_their_prefix()
        │       def test_keys_cannot_escape_into_the_credential_prefix()
        │       def test_backslash_is_refused_rather_than_interpreted()
        │       def test_the_builder_refuses_a_manifest_path_that_escapes()
        ...
        │       def _mode()
        │       def _person()
        │       def test_a_persons_key_is_not_readable_by_anyone_else()
        │       def test_the_nodes_own_key_gets_the_same_treatment()
        │       def test_a_file_is_never_briefly_wider_than_its_final_mode()
        │       def test_files_written_before_this_version_are_repaired()
        │       def test_correct_under_an_unusual_umask()
        ...
        │       def test_crisis_and_shielded_always_require_write_auth()
        ...
        ├── test_viewer_scope.py
        │       def _entry()
        │       def test_visibility_decision_matrix()
        ...
        │       def _service_with_identities()
        │       def _user()
        │       def test_admin_scope_is_identical_to_an_ordinary_users()
        │       def test_anonymous_user_resolves_to_the_empty_scope()
        │       def test_env_key_without_a_did_still_owns_what_it_created()
        └── test_visibility.py
                class _InMemoryManager (__init__)
                class _FakeStorageService (__init__)
...
- `AuthenticationService`
  - Methods: viewer_scope, sign_provenance_if_possible, local_node_scope, get_identity, verify_grant
...
### `src/core/services/domain_bind.py`
> Fetch and validate ``.well-known/ohm-did.json`` for domain bindings....

**Exports:** validate_well_known_document

**Functions:**
- `validate_well_known_document(doc)`
...
### `tests/unit/test_auth_service_registration.py`
> Self-service registration (#403).

A node operator must not be the only way to become someone on a n...
...
### `tests/unit/test_auth_service_spaces.py`
...
- `PackageFetchRequest` (inherits: BaseModel)
- `PackageFetchResponse` (inherits: BaseModel)
...
**Exports:** PackageBuildRequest, PackagePushRequest, PackagePullRequest, PackageZipItem, PackageDownloadZipRequest
...
- `PackageZipItem` (inherits: BaseModel)
  - One package identity for a batch zip download....
- `PackageDownloadZipRequest` (inherits: BaseModel)
...
**Exports:** utc_now, NodeInfo, PackagePointer, CatalogRecord, SyncDigest

**Classes:**
- `NodeInfo` (inherits: BaseModel)
  - Local node metadata exposed via GET /federation/identify....
- `PackagePointer` (inherits: BaseModel)
...
### `src/core/models/attestation.py`
> Durable attestations (federated-identity Slice 6).

An attestation is a signed, backward-looking *fa...

**Exports:** Attestation, AttestationIssue, CertifyRequest, verify_attestation

**Classes:**
- `Attestation` (inherits: BaseModel)
  - Methods: signing_payload
  - A signed, durable fact about a subject....
- `AttestationIssue` (inherits: BaseModel)
  - Request payload for issuing an attestation....
- `CertifyRequest` (inherits: BaseModel)
...
**Exports:** APIKey, APIKeyCreate, APIKeyResponse, AuthenticatedUser, RegistrationCreate
...
Backed today b...
- `RegistrationCreate` (inherits: BaseModel)
  - Request payload for self-service registration....
- `RegistrationResponse` (inherits: BaseModel)
...
### `src/core/models/disclosure.py`
> OKW disclosure profiles — field-group redaction for federation audiences....

**Exports:** DisclosureAudience, DisclosureGroup, AudienceDisclosure, DisclosureProfile, DisclosureBody

**Classes:**
- `DisclosureAudience` (inherits: str, Enum)
- `DisclosureGroup` (inherits: str, Enum)
- `AudienceDisclosure` (inherits: BaseModel)
  - Which field groups are included for one audience....
- `DisclosureProfile` (inherits: BaseModel)
  - Per-facility disclosure config for followers and public audiences....
- `DisclosureBody` (inherits: BaseModel)
  - PUT body for disclosure profiles....
- `DisclosureResponse` (inherits: BaseModel)
- `DisclosurePreviewResponse` (inherits: BaseModel)
  - Redacted facility projection for one federation audience....

**Functions:**
- `default_disclosure_profile()`
- `groups_for_audience(profile, audience)`
- `project_facility(facility)`
...
**Exports:** Credit, RecordProvenance, sign_provenance, verify_provenance, record_attribution
...
### `src/core/models/space.py`
> Space claim model (federated-identity Slice 5).

A space is an :class:`~src.core.models.identity.Ide...

**Exports:** SpaceClaim, SpaceClaimRequest

**Classes:**
- `SpaceClaim` (inherits: BaseModel)
  - Methods: signing_payload
  - Signed statement: ``admin_did`` administers ``space_did``....
- `SpaceClaimRequest` (inherits: BaseModel)
...
### `src/core/models/visibility.py`
> Per-record share / visibility policy (federated-identity Slice 4).

Local publishing policy — who ma...

**Exports:** VisibilityLevel, is_shareable, VisibilityBody, VisibilityResponse, ViewerScope

**Classes:**
- `VisibilityLevel` (inherits: str, Enum)
  - How far a record may leave this node.

``followers`` and ``public`` are both cat...
- `VisibilityBody` (inherits: BaseModel)
  - Request body for PUT …/visibility....
- `VisibilityResponse` (inherits: BaseModel)
  - Visibility for one record....
- `ViewerScope`
  - Methods: owns
  - Which records one caller may see in a list.

A viewer sees shareable records plu...
...
``None`` is t...
- `visible_to(level, viewer, created_by_did)`
...
**Exports:** display_country_name, country_match_key, countries_match, country_code
...
### `src/core/utils/safe_paths.py`
> Containment checks for paths and object keys derived from untrusted input.

Package building takes f...

**Exports:** UnsafePathError, safe_join, safe_key

**Classes:**
- `UnsafePathError` (inherits: ValueError)
...
Resolves both sides be...
- `safe_key(prefix, relative)`
...
**Exports:** SecurityMode, SecurityPolicy, parse_security_mode, get_security_policy, allows_full_metadata_logging

**Classes:**
- `SecurityMode` (inherits: str, Enum)
  - Deployment security posture....
- `SecurityPolicy`
  - Methods: to_public_dict
...
- `allows_full_metadata_logging(policy)`
...
- `TestDeployEnvVars`
  - Methods: test_production_applies_storage_target, test_staging_enables_federation_under_the_names_settings_reads, test_staging_federation_makes_no_outbound_peer_calls, test_development_is_local, test_missing_environment_returns_empty
...
### `deploy/providers/azure/app_secrets.py`
> Secrets the Celery worker shares with the API container app.

Container App secrets are per-app, so ...

**Exports:** shared_secret_env_refs, mirrored_secret_names, mirrored_secret_env_vars

**Functions:**
- `shared_secret_env_refs()`
...
One Azure Cache for Redis instance backs...

**Exports:** RedisConfigError, build_redis_secret_values, redis_secret_env_vars

**Classes:**
- `RedisConfigError` (inherits: ValueError)
  - Raised when the [redis] table is missing fields needed to build a URL....

**Functions:**
- `build_redis_secret_values(redis_config, access_key)`
...
### `deploy/scripts/deploy_azure_worker.py`
> Deploy the OHM **Celery worker** container to Azure Container Apps.

The worker runs the SAME image ...
...
### `harness/modules/probe_async_generation.py`
> Probe: async generate-from-url jobs actually complete.

This is the health signal for the Celery wor...

**Exports:** ProbeAsyncGenerationLoop

**Classes:**
- `ProbeAsyncGenerationLoop` (inherits: ProbeModule)
  - Methods: discover, observe, judge
...
Copies ``docs-site/docs/`` (authored markdown) in...
...
### `scripts/generate_demo_world.py`
> Emit the frontend's demo world from the canonical seed dataset.

There are two ways to get a demo wo...
...
Why this exists, given ``...

**Exports:** demo_id, build_okh, build_okw, coverage, clear_demo_records
...
- `clear_demo_records(storage_dir)`
...
**Exports:** okw_group, visibility_group, disclosure_group, okw_template, create_interactive
...
**Exports:** require_permission, created_by, created_by_did
...
### `src/core/federation/okw_catalog.py`
> Signed OKW federation catalog (separate Merkle tree from OKH)....

**Exports:** OkwCatalogRecord, SignedOkwRecord, OkwCatalogIndex

**Classes:**
- `OkwCatalogRecord` (inherits: BaseModel)
  - Methods: record_payload
  - Signed index entry for one redacted OKW facility projection....
- `SignedOkwRecord` (inherits: BaseModel)
- `OkwCatalogIndex`
  - Methods: get_signed_record

### `src/core/federation/okw_ingest.py`
> Validate and persist redacted OKW projections from followed peers....

**Exports:** OkwIngestError, OkwIngestResult, verify_signed_okw_record

**Classes:**
- `OkwIngestError` (inherits: Exception)
...
### `src/core/federation/okw_sync.py`
> Anti-entropy sync for the OKW catalog (separate from OKH)....

**Exports:** OkwSyncPeerResult

**Classes:**
- `OkwSyncPeerResult`

### `src/core/federation/package_fetch.py`
> On-demand package artifact fetch from followed peers (CAS channel)....

**Exports:** PackageFetchResult

**Classes:**
- `PackageFetchResult`

### `src/core/federation/package_pointer.py`
> Resolve local package artifacts for federation catalog pointers....

**Exports:** find_package_dir_for_manifest, resolve_package_pointer, find_package_dir_by_bundle_hash, package_dir_to_archive_bytes, write_fetched_package

**Functions:**
- `find_package_dir_for_manifest(manifest_id)`
  - Return the package directory whose okh-manifest.json matches ``manifest_id``....
- `resolve_package_pointer(manifest_id)`
  - Build a catalog package pointer if a local package exists for the design....
- `find_package_dir_by_bundle_hash(bundle_hash_value)`
  - Locate a local package directory whose pin-derived bundle hash matches....
- `package_dir_to_archive_bytes(package_dir)`
  - Return (bytes, filename) for serving — prefer existing zip, else tar.gz....
- `write_fetched_package(data)`
...
**Exports:** bundle_hash, create_pin_record, verify_pin_record, load_pin_record

**Functions:**
- `bundle_hash(pin_record)`
...
### `src/core/storage/attestation_store.py`
> Attestation store (federated-identity Slice 6).

Durable signed facts about subjects / content hashe...

**Exports:** AttestationStore
...
### `src/core/storage/binding_store.py`
> Identity binding + trust-on-follow directory stores (Slice 7)....

**Exports:** BindingStore, DirectoryStore
...
### `src/core/storage/disclosure_store.py`
> Per-facility OKW disclosure profile store....

**Exports:** DisclosureStore
...
### `src/core/storage/space_claim_store.py`
> Space-claim store (federated-identity Slice 5).

TOFU admin bindings for space DIDs. Not secret; one...

**Exports:** SpaceClaimStore

**Classes:**
- `SpaceClaimStore`
  - Object-store persistence for space claims, keyed by space_did....

### `src/core/storage/visibility_store.py`
> Per-record visibility store (federated-identity Slice 4).

Local publishing policy keyed by record i...

**Exports:** VisibilityStore
...
**Exports:** ProcessDefinition, load_from_yaml, validate_definitions, ProcessTaxonomy, canonical_processes
...
### `tests/api/test_okw_spaceapi.py`
> Publishing a facility outward to Maps of Making.

Two things are being pinned here. The document mus...

**Exports:** fixture, test_capabilities_land_in_the_key_mom_reads, test_unrecognized_processes_are_dropped_from_the_tags, test_no_street_address_or_contact_is_published, test_country_code_is_alpha2_and_omitted_when_unknown

**Functions:**
- `fixture()`
  - A facility carrying details that must not reach the map....
- `test_capabilities_land_in_the_key_mom_reads()`
  - MoM builds its specialty chips from knowsAbout, kebab-cased.

Emitting process d...
- `test_unrecognized_processes_are_dropped_from_the_tags()`
- `test_no_street_address_or_contact_is_published()`
  - The document reaches a store we do not control, so it carries less.

`location` ...
- `test_country_code_is_alpha2_and_omitted_when_unknown()`

**Internal Dependencies:** 9 imports

### `tests/api/test_package_build_response.py`
...
### `tests/api/test_package_download_zip.py`
> Contract tests for POST /api/package/download-zip....

**Exports:** mock_two_packages

**Functions:**
- `mock_two_packages(tmp_path)`
...
### `tests/api/test_read_visibility.py`
> Private records must not be served to unauthenticated callers.

docs-site/docs/guides/who-can-see-yo...
...
**Exports:** test_identity_group_exposes_subcommands, test_spaces_subgroup_commands, test_attestations_subgroup_commands, test_bindings_subgroup_commands, test_directory_subgroup_commands

**Functions:**
- `test_identity_group_exposes_subcommands()`
- `test_spaces_subgroup_commands()`
- `test_attestations_subgroup_commands()`
- `test_bindings_subgroup_commands()`
...
**Exports:** test_okh_exposes_create_and_visibility, test_okh_create_has_author_flags, test_okh_visibility_subgroup, test_okw_exposes_create_visibility_and_disclosure, test_okw_create_has_author_flags

**Functions:**
- `test_okh_exposes_create_and_visibility()`
- `test_okh_create_has_author_flags()`
- `test_okh_visibility_subgroup()`
- `test_okw_exposes_create_visibility_and_disclosure()`
- `test_okw_create_has_author_flags()`
...
### `tests/federation/test_disclosure.py`
> Unit tests for OKW disclosure profiles and projection....

**Exports:** test_default_profile_is_identity_only, test_project_fail_closed_strips_location_and_equipment, test_project_with_location_group, test_groups_for_audience_always_includes_identity

**Functions:**
- `test_default_profile_is_identity_only()`
- `test_project_fail_closed_strips_location_and_equipment()`
- `test_project_with_location_group()`
- `test_groups_for_audience_always_includes_identity()`
...
**Exports:** test_verify_signed_record_accepts_valid, test_verify_signed_record_rejects_bad_manifest_sig, test_verify_signed_record_accepts_valid_provenance, test_verify_signed_record_rejects_tampered_provenance, test_verify_signed_record_rejects_bad_attestation

**Functions:**
- `test_verify_signed_record_accepts_valid()`
- `test_verify_signed_record_rejects_bad_manifest_sig()`
- `test_verify_signed_record_accepts_valid_provenance()`
- `test_verify_signed_record_rejects_tampered_provenance()`
- `test_verify_signed_record_rejects_bad_attestation()`
...
### `tests/federation/test_okw_catalog.py`
> OKW federation catalog builds redacted projections....

**Internal Dependencies:** 6 imports

### `tests/federation/test_okw_ingest_provenance.py`
> OKW federation ingest stamps synced provenance for the UI banner....

**Internal Dependencies:** 5 imports

### `tests/federation/test_package_pointer.py`
> Tests for federation package pointers and CAS helpers....

**Exports:** test_resolve_package_pointer_from_local_package, test_resolve_package_pointer_absent, test_manifest_hash_unchanged_by_package_pointer_fields, test_try_extract_rejects_zip_slip

**Functions:**
- `test_resolve_package_pointer_from_local_package(tmp_path, monkeypatch)`
- `test_resolve_package_pointer_absent(monkeypatch)`
- `test_manifest_hash_unchanged_by_package_pointer_fields()`
  - Package pointer must not alter design content hash....
- `test_try_extract_rejects_zip_slip(tmp_path)`
...
### `tests/integration/test_list_visibility_field.py`
> List items carry their visibility, so the UI can find what is not yet shared.

Post-#403 a caller se...

**Exports:** test_okh_list_reports_visibility_and_hides_it_from_others, test_okw_list_reports_visibility

**Functions:**
- `test_okh_list_reports_visibility_and_hides_it_from_others(client)`
- `test_okw_list_reports_visibility(client)`
...
### `tests/matching/taxonomy_coverage.py`
> Taxonomy ↔ facility matchability coverage helpers....

**Exports:** ProcessCoverageRow, iter_canonical_ids, facilities_claiming_process, minimal_okh_for_process, format_coverage_table
...
Wikidata QIDs are the share...

**Exports:** test_every_taxonomy_qid_resolves_to_a_plausible_entity

**Functions:**
- `test_every_taxonomy_qid_resolves_to_a_plausible_entity()`
...
- `actual_fe_routes(app_tsx, app_dir)`
  - Route prefixes declared by the frontend router.

Next App Router tree (``fronten...
...
**Exports:** Area, Endpoint, uncalled_endpoint_paths, endpoints_by_status, SiteDoc
...
- `endpoints_by_status(status)`
- `doc_areas()`
  - Area names a guides/ page may legitimately declare....
- `site_doc_paths()`
  - guides/ page paths declared by SITE_DOCS....
- `site_doc_for_path(path)`
  - The SiteDoc row documenting ``path``, or None if undeclared....

### `tests/parity/test_api_coverage.py`
> Frontend <-> API endpoint coverage gate.

``test_parity.py`` checks that the frontend touches each A...

**Exports:** test_scan_finds_the_frontend_api_surface, test_every_api_path_is_called_or_recorded, test_recorded_endpoints_are_not_actually_called, test_recorded_endpoints_still_exist, test_endpoint_rows_are_coherent

**Functions:**
- `test_scan_finds_the_frontend_api_surface()`
  - The scanner must actually find calls, or every other test is vacuous....
- `test_every_api_path_is_called_or_recorded()`
  - No endpoint may be unaccounted for. This is the ratchet....
- `test_recorded_endpoints_are_not_actually_called()`
  - A row cannot rot: the frontend calling it must fail this gate.

For a `planned` ...
- `test_recorded_endpoints_still_exist()`
  - A rename or deletion must not leave a row pointing at nothing....
- `test_endpoint_rows_are_coherent()`
  - The table's own shape, checked the way AREAS' slots are....

### `tests/parity/test_compose_defaults.py`
> Compose defaults may not silently contradict the configuration schema.

`docker-compose.yml` restate...

**Exports:** test_compose_defaults_match_the_schema_unless_declared, test_the_llm_kill_switch_is_not_inverted, test_copying_the_env_template_cannot_disable_background_import, test_every_declared_override_is_still_a_real_divergence

**Functions:**
- `test_compose_defaults_match_the_schema_unless_declared(service)`
- `test_the_llm_kill_switch_is_not_inverted()`
  - The specific regression above, pinned by name.

A self-hoster adding a key throu...
- `test_copying_the_env_template_cannot_disable_background_import(service)`
  - `cp .env.example .env` is the documented first step of configuring a node.

It i...
- `test_every_declared_override_is_still_a_real_divergence()`
...
The site at openhardwaremanager.org states what works to...

**Exports:** test_r1_site_docs_declare_valid_status_and_area, test_r1_site_docs_have_no_duplicate_pages, test_r2_guides_pages_declare_area_and_surface, test_r3_non_guides_pages_do_not_declare_an_area, test_r4_declared_pages_exist

**Functions:**
- `test_r1_site_docs_declare_valid_status_and_area()`
- `test_r1_site_docs_have_no_duplicate_pages()`
- `test_r2_guides_pages_declare_area_and_surface()`
- `test_r3_non_guides_pages_do_not_declare_an_area()`
  - Narrative and reference pages are parity-exempt, structurally.

Enforced in the ...
- `test_r4_declared_pages_exist()`
...
### `tests/parity/test_viewer_scope_ratchet.py`
> Every request handler that lists records must say whose view it is (#403).

``list(viewer=None)`` is...

**Exports:** test_no_route_lists_records_without_a_viewer_scope, test_the_ratchet_can_actually_fail

**Functions:**
- `test_no_route_lists_records_without_a_viewer_scope()`
- `test_the_ratchet_can_actually_fail()`
...
**Exports:** service, test_bundle_hash_is_merkle_over_manifest_and_sorted_files, test_verify_attestation_roundtrip
...
**Functions:**
- `service(tmp_path)`
- `test_bundle_hash_is_merkle_over_manifest_and_sorted_files()`
- `test_verify_attestation_roundtrip()`
...
### `tests/unit/test_azure_redis_secrets.py`
> Unit tests for minting Redis connection URLs as Container App secrets.

No live Azure calls — subpro...

**Exports:** test_builds_one_url_per_database, test_every_url_requires_tls_verification, test_access_key_is_percent_encoded, test_encoded_key_round_trips_through_a_url_parser, test_incomplete_config_is_refused

**Functions:**
- `test_builds_one_url_per_database()`
- `test_every_url_requires_tls_verification()`
  - A bare rediss:// URL parses to CERT_NONE under kombu — silently insecure....
- `test_access_key_is_percent_encoded()`
  - Azure Redis keys are base64: '/' and '+' appear and MUST be encoded.

An unencod...
- `test_encoded_key_round_trips_through_a_url_parser()`
- `test_incomplete_config_is_refused(missing)`

**Internal Dependencies:** 2 imports

### `tests/unit/test_azure_worker_deploy.py`
> Unit tests for deploying the Celery worker container app.

No live Azure calls — subprocess.run is m...

**Exports:** test_worker_inherits_the_shared_storage_target, test_worker_env_layers_the_worker_table_on_top, test_worker_deploy_shape_is_one_replica_of_the_api_size, test_worker_shape_excludes_the_env_subtable, test_environment_without_a_worker_table_yields_nothing

**Functions:**
- `test_worker_inherits_the_shared_storage_target()`
  - Declared once, so an API and worker cannot point at different data....
- `test_worker_env_layers_the_worker_table_on_top()`
- `test_worker_deploy_shape_is_one_replica_of_the_api_size()`
- `test_worker_shape_excludes_the_env_subtable()`
  - [worker.env] is container env, not deploy shape....
- `test_environment_without_a_worker_table_yields_nothing()`
...
**Exports:** service, test_domain_external_id_normalizes_host, test_oauth_external_id
...
**Functions:**
- `service(tmp_path)`
- `test_domain_external_id_normalizes_host()`
- `test_oauth_external_id()`

**Internal Dependencies:** 15 imports

### `tests/unit/test_bom_nlp_cost.py`
> Guards against re-introducing full spaCy pipeline runs in BOM collection.

Generation on one real re...

**Exports:** collector, test_bom_likelihood_uses_the_tokenizer_not_the_pipeline, test_bom_likelihood_still_scores_on_keywords, test_section_finding_does_not_parse_the_document, test_no_nlp_available_is_handled
...
Calling it (``nlp(text)``) means the full pipel...
- `_Token`

**Functions:**
- `collector()`
- `test_bom_likelihood_uses_the_tokenizer_not_the_pipeline(collector)`
- `test_bom_likelihood_still_scores_on_keywords(collector)`
  - Cheaper must not mean inert — keyword hits still raise the score....
- `test_section_finding_does_not_parse_the_document(collector)`
  - Sectioning is textual; it must not parse the document to do it....
- `test_no_nlp_available_is_handled(collector)`
  - Both paths degrade rather than raising when spaCy is unavailable....

**Internal Dependencies:** 1 imports

### `tests/unit/test_cache_redis_backend.py`
...
### `tests/unit/test_docs_site_build.py`
> Unit tests for the public docs staging step (scripts/build_docs_site.py).

The staging step is what ...

**Exports:** build_docs_site, test_badge_lands_directly_under_the_h1, test_badge_text_differs_per_status, test_page_without_an_h1_still_gets_a_badge, test_unknown_status_leaves_the_page_untouched

**Functions:**
- `build_docs_site()`
- `test_badge_lands_directly_under_the_h1(build_docs_site)`
- `test_badge_text_differs_per_status(build_docs_site)`
- `test_page_without_an_h1_still_gets_a_badge(build_docs_site)`
...
These are ordering and gating p...

**Exports:** workflow, test_worker_deploys_before_the_api, test_worker_and_api_deploy_the_same_digest, test_worker_deploy_is_gated_like_the_api_deploy, test_worker_deploy_verifies_the_rolled_image

**Functions:**
- `workflow()`
- `test_worker_deploys_before_the_api(workflow)`
  - Consumer before producer: the API must never enqueue work for stale code....
- `test_worker_and_api_deploy_the_same_digest(workflow)`
  - Pinned to one digest, so the two cannot run different code....
- `test_worker_deploy_is_gated_like_the_api_deploy(workflow)`
  - One production approval covers the release; no unguarded side door....
- `test_worker_deploy_verifies_the_rolled_image(workflow)`
...
### `tests/unit/test_repo_map_generation.py`
> Tests for the .repo-map.md generator.

The map is a generated file gated on byte-equality, so the pr...

**Exports:** test_identity_comes_from_the_project_name, test_identity_falls_back_to_the_directory_without_a_pyproject, test_identity_falls_back_when_the_pyproject_is_unreadable, test_identity_falls_back_when_the_project_name_is_missing, test_map_does_not_depend_on_the_checkout_directory_name

**Functions:**
- `test_identity_comes_from_the_project_name(tmp_path)`
- `test_identity_falls_back_to_the_directory_without_a_pyproject(tmp_path)`
- `test_identity_falls_back_when_the_pyproject_is_unreadable(tmp_path)`
- `test_identity_falls_back_when_the_project_name_is_missing(tmp_path)`
- `test_map_does_not_depend_on_the_checkout_directory_name(tmp_path)`
...
### `tests/unit/test_secret_file_permissions.py`
> Private key material is never written world-readable (#410).

Plaintext-at-rest is a documented peac...

**Exports:** test_a_persons_key_is_not_readable_by_anyone_else, test_the_nodes_own_key_gets_the_same_treatment, test_a_file_is_never_briefly_wider_than_its_final_mode, test_files_written_before_this_version_are_repaired, test_correct_under_an_unusual_umask

**Functions:**
- `test_a_persons_key_is_not_readable_by_anyone_else(tmp_path)`
- `test_the_nodes_own_key_gets_the_same_treatment(tmp_path)`
  - The rule is 'this directory holds secrets', not 'this filename is special'....
- `test_a_file_is_never_briefly_wider_than_its_final_mode(tmp_path)`
  - A create-then-chmod leaves a window in which the key is readable, which
is the w...
- `test_files_written_before_this_version_are_repaired(tmp_path)`
  - An upgrade must not leave old installations exposed....
- `test_correct_under_an_unusual_umask(tmp_path, umask)`
...
**Exports:** test_peacetime_is_default, test_peacetime_preset_values, test_crisis_preset_values, test_shielded_preset_values, test_policy_is_immutable

**Functions:**
- `test_peacetime_is_default()`
- `test_peacetime_preset_values()`
- `test_crisis_preset_values()`
- `test_shielded_preset_values()`
...
### `tests/unit/test_staging_deploy.py`
...
### `tests/unit/test_viewer_scope.py`
> Record ownership is scoped per viewer, and keys on the subject DID (#403).

Before this, list endpoi...

**Exports:** test_visibility_decision_matrix, test_account_fallback_covers_records_and_keys_that_have_no_did, test_a_record_with_no_attribution_at_all_is_owned_by_nobody, test_admin_scope_is_identical_to_an_ordinary_users, test_anonymous_user_resolves_to_the_empty_scope

**Functions:**
- `test_visibility_decision_matrix(level, scope, expected)`
- `test_account_fallback_covers_records_and_keys_that_have_no_did()`
  - Env keys carry no DID, and pre-#403 records carry only account attribution....
- `test_a_record_with_no_attribution_at_all_is_owned_by_nobody()`
- `test_admin_scope_is_identical_to_an_ordinary_users(tmp_path)`
  - `admin` must buy nothing here — that is the whole boundary (ADR §9)....
- `test_anonymous_user_resolves_to_the_empty_scope(tmp_path)`
...
**Exports:** test_default_visibility_is_private, test_is_shareable
