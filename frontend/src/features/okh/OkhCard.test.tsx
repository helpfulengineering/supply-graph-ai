...
function baseOkh(overrides: Partial<OkhManifest> = {}): OkhManifest {
  return {
    ...
    repo: null,
    function:
      "A long function text that should not appear on the card anymore.",
    description:
      "A long description that should not appear on the card anymore.",
    intended_use: null,
...
describe("OkhCard", () => {
  it("shows Title Case title, category, processes, author, version, license", () => {
    render(<OkhCard okh={baseOkh()} />);

    expect(
      screen.getByRole("heading", { name: "3D Simple Bias Tape Maker" }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("button", { name: /Run Match/i }),
    ).toBeInTheDocument();
  });
