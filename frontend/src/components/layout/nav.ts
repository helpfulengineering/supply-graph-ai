...
/**
 * The sitemap as the selected matching domain presents it.
 *
 * The cooking domain is not a separate set of routes — it is the same `/okh`
 * and `/facilities` browsing a different catalog, which is why `NAV_GROUPS`
 * above stays the manufacturing sitemap and this is a transform over it rather
 * than a second table. `navEntryFor` and `e2e/chrome.spec.ts` keep reading the
 * table directly, so nothing that resolves a route has to learn about domains.
 *
 * Cooking renames the two browse rows and drops the rest. The authoring group
 * goes entirely: Generate drafts an OKH manifest from a URL, New design and New
 * facility author OKW/OKH records, and RFQ quotes a manufacturing solution —
 * none of them have a cooking equivalent yet. Match is reachable at /match and
 * guarded there; it is absent from the menu because a match needs a recipe
 * chosen first, and the recipe list is where you choose one.
 *
 * This replaces `navItemsForDomain` in features/settings/domainPreference,
 * which carried the same intent against the pre-migration flat nav model.
 */
export function navGroupsForDomain(
  domain: "manufacturing" | "cooking",
  groups: NavGroup[] = NAV_GROUPS,
): NavGroup[] {
  if (domain !== "cooking") return groups;
  const COOKING_LABELS: Record<string, { name: string; desc: string }> = {
    "/okh": {
      name: "Recipes",
      desc: "browse the recipes this instance can match",
    },
    "/facilities": {
      name: "Kitchens",
      desc: "kitchens available for matching",
    },
  };
  return groups
    .map((group) => ({
      ...
      entries: group.entries.flatMap((entry) => {
        const relabel = COOKING_LABELS[entry.href];
        if (relabel) return [{ ...entry, ...relabel }];
        // Reference survives unchanged; it is about the app, not the domain.
        return group.label === "Reference" ? [entry] : [];
      }),
    }))
