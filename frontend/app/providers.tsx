...
function MountedProviders({ children }: { children: ReactNode }) {
  ...
  // The matching domain is a browser-local preference that reshapes nav and
  // swaps the catalog views, so it has to sit above the routed subtree exactly
  // as the theme does. It lived in App.tsx, which the Next migration deletes;
  // without this mount every useDomain() consumer reads the context default
  // and the cooking domain silently disappears.
