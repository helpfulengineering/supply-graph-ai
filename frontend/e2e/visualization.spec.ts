...
test("supply-tree explorer renders KPIs and the tree (mocked)", async ({
  page,
}, testInfo) => {
...
test("shows production sequence and dependencies (mocked)", async ({
  page,
}, testInfo) => {
  test.skip(testInfo.project.name === "real-api", "uses a fixture solution id");
  await page.goto("/visualization/sol-1");
  await expect(
    page.getByRole("heading", { name: /production sequence/i }),
  ).toBeVisible();
...
test("downloads the solution as a JSON file (mocked)", async ({
  page,
}, testInfo) => {
  test.skip(testInfo.project.name === "real-api", "uses a fixture solution id");
  await page.goto("/visualization/sol-1");
