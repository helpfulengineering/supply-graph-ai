...
test("match page loads with design search and expanded facility filters", async ({
  ...
  test.skip(
    ...
    "expects populated catalog UI",
  );
  await page.goto("/match");
  await expect(
    page.getByRole("heading", { name: /match a design/i }),
  ).toBeVisible();
...
test("System Mode selector controls the match request (mocked)", async ({
  page,
}, testInfo) => {
...
test("Maps of Making source shows MoM facilities (mocked)", async ({
  page,
}, testInfo) => {
  test.skip(testInfo.project.name === "real-api", "asserts fixture data");
  await page.goto("/match");
...
test("network mode sends network_filter and shows the banner (mocked)", async ({
  ...
  expect(body!.network_filter).toMatchObject({
    country: "FR",
...
test("near-misses are filtered by a tolerance the design's size bounds (mocked)", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "Complete Works" }),
  ).toBeVisible();
  await expect(
