            ...
            def run_in_thread():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    ...
                    return future.result(
                        timeout=70
                    )  # 70 second timeout for facilities loading
...
def facilities_to_dataframe(facilities: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    for facility in facilities:
        ...
        if not gps_str:
            skipped_count += 1
            logger.debug(
                f"Skipping facility '{facility.get('name')}': No GPS coordinates"
            )
            continue
        ...
        if not coords:
            skipped_count += 1
            logger.debug(
                f"Skipping facility '{facility.get('name')}': Invalid GPS format '{gps_str}'"
            )
            continue
        ...
        rows.append(
            {
                ...
                or location.get("address", {}).get("country"),
                "facility_status": facility.get("facility_status", "Unknown"),
            }
        )

    logger.info(
        f"Processed {len(facilities)} facilities: {len(rows)} with GPS, {skipped_count} skipped"
    )

    if not rows:
        # Return empty DataFrame with correct columns
        return pd.DataFrame(
            columns=["lat", "lon", "name", "city", "country", "facility_status"]
        )
...
def get_facilities_for_map() -> pd.DataFrame:
    ...
    if facilities:
        logger.info(
            f"Sample facility keys: {list(facilities[0].keys()) if facilities else 'None'}"
        )
