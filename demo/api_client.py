...
class APIClient:
    ...
    async def get_okh_designs(
        self, page: int = 1, page_size: int = 100, timeout: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            return await self._request(
                "GET", "/v1/api/okh", params={"page": page, "page_size": page_size}
            )
        finally:
    ...
    async def match_facilities(
        ...
        try:
            return await self._request("POST", "/v1/api/match", json_data=payload)
        finally:
            # Restore original timeout
            self.timeout = original_timeout
