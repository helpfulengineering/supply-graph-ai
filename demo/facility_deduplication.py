...
def deduplicate_facilities(
    ...
    filtered_trees = [
        tree for tree in all_trees if tree.get("confidence_score", 0.0) > min_confidence
    ]
    ...
    for facility_name, trees in facilities_dict.items():
        ...
        for tree in trees:
            ...
            if component_id:
                if component_id not in components_by_id:
                    ...
                    existing_score = components_by_id[component_id].get(
        ...
        if aggregate_score_method == "highest":
            ...
            raise ValueError(
                f"Unknown aggregate_score_method: {aggregate_score_method}"
            )
...
def get_alternative_facilities_for_component(
    ...
    alternatives.sort(key=lambda t: t.get("confidence_score", 0.0), reverse=True)
