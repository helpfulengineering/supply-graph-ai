...
def select_best_solution(
    data: Dict[str, Any], selector_name: str = None
) -> Dict[str, Any]:
    """
            ...
            facilities_in_solution.append(
                {
                    ...
                    "component_id": (
                        facility["components"][0].get("component_id")
                        if facility["components"]
                        else None
                    ),
                    "component_name": (
                        facility["components"][0].get("component_name")
                        if facility["components"]
                        else None
                    ),
            ...
            facilities_in_solution.append(
                {
                    ...
                    "depends_on": list(
                        set(dep for t in trees for dep in t.get("depends_on", []))
                    ),
...
def get_component_name_for_tree(tree: Dict[str, Any], name_map: Dict[str, str]) -> str:
    """
...
def parse_single_level_solutions(
    ...
    if use_selection:
        ...
        selection_criteria = selected_data.get(
            "selection_criteria", selector_name or DEFAULT_SELECTOR
        )
        ...
        formatted.append(
            {
                ...
                "total_facilities_available": len(
                    facility_details_all
                ),  # For info display
    ...
    for facility_set, components in facility_set_to_components.items():
        if len(components) > 1:
            duplicate_groups.append(
                [c["component_name"] or c["component_id"] for c in components]
            )
