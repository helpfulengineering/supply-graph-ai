...
class SolutionSelector(ABC):
    ...
    def _count_unique_facilities(self, solution: Dict[str, Any]) -> int:
        ...
        if not all_trees:
            ...
            if tree:
                facility_name = tree.get("facility_name") or solution.get(
                    "facility_name"
                )
    ...
    def _get_average_component_score(self, solution: Dict[str, Any]) -> float:
        ...
        if not all_trees:
            ...
            if tree:
                return tree.get(
                    "confidence_score",
                    solution.get("confidence", solution.get("score", 0.0)),
                )
            # Fallback to solution score/confidence
            return solution.get("score", solution.get("confidence", 0.0))

        total_score = sum(tree.get("confidence_score", 0.0) for tree in all_trees)
