...
def get_design_metadata(design: Dict[str, Any]) -> Dict[str, Any]:
    ...
    sub_parts_count = (
        _count_sub_parts_recursive(sub_parts) if isinstance(sub_parts, list) else 0
    )
...
def get_okh_designs_for_selection() -> List[Dict[str, Any]]:
    ...
    for design in designs:
        try:
            ...
            logger.warning(
                f"Failed to process design {design.get('id', 'unknown')}: {e}"
            )
            continue
...
def get_design_by_id(
    design_id: str, designs: List[Dict[str, Any]]
