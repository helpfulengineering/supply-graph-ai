            ...
            if isinstance(item, dict):
                ...
                if equipment_type:
                    if "wikipedia.org/wiki/" in equipment_type:
                        eq_name = (
                            equipment_type.split("/wiki/")[-1].replace("_", " ").title()
                        )
                    else:
...
def main():
    ...
    st.markdown(
        "OKW facilities displayed on an interactive map. Click on a pin to view facility details."
    )
    ...
    with st.spinner("Loading facilities from API... This may take a moment."):
        try:
            ...
            if facilities_df.empty:
                ...
                st.markdown(
                    "- Facilities don't have GPS coordinates in the `gps_coordinates` field"
                )
                st.markdown(
                    "- There was an error loading data (check Debug Info and terminal logs)"
                )
                st.markdown(
                    "- Async loading failed (check for uvloop/nest_asyncio errors)"
                )
            else:
                ...
                st.caption(
                    "Select a facility from the dropdown to view detailed information"
                )
                ...
                if (
                    selected_facility_label
                    and selected_facility_label != "Select a facility..."
                ):
                    # Extract facility name from label
                    facility_name = selected_facility_label.split(" (")[0]

                    # Find matching facility in DataFrame
                    matching_facility = facilities_df[
                        facilities_df["name"] == facility_name
                    ]
                    ...
                    if not matching_facility.empty:
                        ...
                        lookup_key = (
                            f"{facility_name}|{facility_city}|{facility_country}"
                        )
                        ...
                        if full_facility:
                            st.markdown("---")
                            with st.expander(
                                f"📍 **{facility_name}** - Facility Details",
                                expanded=True,
                            ):
                                _display_facility_details(full_facility)
                        else:
                            st.warning(
                                f"Full facility data not found for {facility_name}"
                            )
                    else:
    ...
    with st.spinner("Loading OKH designs... This may take a moment."):
        try:
            ...
            if not designs:
                st.warning(
                    "No OKH designs found. Please ensure the API is accessible and data is loaded."
                )
                st.info("**Possible causes:**")
                ...
                design_options = {
                    f"{d['name']} ({d['complexity']}, {d['component_count']} components)": d[
                        "id"
                    ]
                    for d in designs
                }
                ...
                if selected_design:
                    ...
                    if selected_design["description"]:
                        st.markdown(
                            f"**Description:** {selected_design['description']}"
                        )
                ...
                with col1:
                    ...
                    location_by_city = (
                        location_by_city.strip() if location_by_city else None
                    )

                    # Distance filter (simplified - just lat/lon for now)
                    use_distance_filter = st.checkbox(
                        "Filter by distance",
                        help="Filter facilities by distance from a location",
                    )
                    ...
                    if use_distance_filter:
                        ...
                        radius_km = st.number_input(
                            "Radius (km):", min_value=1, value=100, step=10
                        )
                ...
                if st.button(
                    "Match Facilities", type="primary", use_container_width=True
                ):
                    # Store selected design and parameters in session state
                    if selected_design:
                        with st.spinner(
                            "Matching facilities... This may take 30-60 seconds for complex designs."
                        ):
                            try:
                                ...
                                if max_depth is None or max_depth == 0:
                                    ...
                                    if has_nested_components(okh_manifest):
                                        ...
                                        st.info(
                                            "🔗 **Nested design detected**: Auto-enabling multi-facility matching"
                                        )
                                ...
                                if (
                                    ...
                                    st.warning(
                                        "**Validation Error**: The request format may be incorrect. Check that:"
                                    )
                                    ...
                                    st.warning(
                                        "**Timeout Error**: The matching operation took too long."
                                    )
                                    st.markdown(
                                        "- Try reducing the complexity (select a simpler design)"
                                    )
                                    st.markdown("- Try reducing `max_depth` if set")
                                    st.markdown(
                                        "- Try increasing `min_confidence` to reduce matches"
                                    )
                                    st.markdown(
                                ...
                                st.info(
                                    "💡 **Tip**: Check terminal logs for detailed error messages"
                                )
                    else:
    ...
    if "matching_result" in st.session_state:
        ...
        st.success(
            f"✅ Matching completed for: **{params.get('design_name', 'Unknown Design')}**"
        )
        ...
        if result.get("status") == "success":
            ...
            if not selected_design and "selected_design_id" in st.session_state:
                # Get designs list
                designs = get_okh_designs_for_selection()
                selected_design = get_design_by_id(
                    st.session_state["selected_design_id"], designs
                )
            ...
            if matching_mode == "single-level":
                ...
                if len(solutions) > 1:
                    ...
                    st.info(
                        f"📊 **Selected**: {selected_option} (showing 1 of {len(solutions)} solutions)"
                    )
            ...
            if matching_mode == "nested":
                ...
                parsed = parse_nested_solution(
                    data, use_deduplication=True, aggregate_score_method="highest"
                )
                ...
                deduplicated_facilities = parsed.get(
                ...
                st.info(
                    f"🔗 **Multi-Facility Solution Detected**: This design requires **{facility_count} facilities** working together. No single facility can produce the entire design."
                )
                ...
                if component_mapping:
                    ...
                    if selected_design and "full_design" in selected_design:
                        comp_name_map = get_component_name_map(
                            selected_design["full_design"]
                        )
                    ...
                    if deduplicated_facilities:
                        selected_facility_names = {
                            f["name"] for f in deduplicated_facilities
                        }
                        ...
                        selected_facility_names = {
                            f["name"] for f in facilities_in_solution
                        }
                    ...
                    if formatted_mapping:
                        ...
                        st.markdown(
                            "Shows which facilities in the selected solution can produce each component."
                        )
                        st.caption(
                        ...
                        duplicate_info = detect_duplicate_component_mappings(
                            formatted_mapping
                        )
                        ...
                        for comp_info in formatted_mapping:
                            ...
                            if facilities:
                                ...
                                if comp_name == comp_id:
                                    ...
                                    st.caption(
                                        "⚠️ Component name not found. Showing ID instead."
                                    )
                                else:
                                ...
                                total_available = comp_info.get(
                                    "total_facilities_available", len(facilities)
                                )
                                facilities_shown = comp_info.get(
                                    "facilities_shown", len(facilities)
                                )
                                ...
                                if len(facilities) == 1:
                                    ...
                                    st.markdown(
                                        f"  - **Facilities** ({facilities_shown}): {', '.join(facilities)}"
                                    )
                                ...
                                if comp_name != comp_id:
                                    ...
                                    st.markdown(f"**Component** (ID: `{comp_id}`)")
                                st.caption(
                                    "⚠️ No facilities in selected solution for this component."
                                )
                                st.markdown("")
                ...
                parsed = parse_single_level_solutions(
                    data, use_selection=True, selector_name=selector_name
                )
                # Store parsed data for RFQ generation
                st.session_state["parsed_matching_data"] = parsed
            ...
            if display_facility_count == 0:
                ...
                st.markdown(
                    "- Process name mismatch (OKH uses '3DP', OKW uses Wikipedia URLs)"
                )
                st.markdown(
                    "- Confidence threshold too high (current: {})".format(
                ...
                if matching_mode == "nested":
                    ...
                    st.markdown(
                        f"**Solution Score:** {solution_score:.2f} (aggregate confidence across all facilities)"
                    )
                    ...
                    if deduplicated_facilities:
                        ...
                        st.info(
                            "🏭 Each facility is shown once with all components it produces listed below."
                        )
                            ...
                            with st.expander(title, expanded=(idx == 1)):
                                ...
                                with col2:
                                    ...
                                    if location_info:
                                        city = (
                                            ...
                                            or location_info.get("address", {}).get(
                                                "city"
                                            )
                                            or "N/A"
                                        )
                                    ...
                                    gps = (
                                        location_info.get("gps_coordinates")
                                        if location_info
                                        else None
                                    )
                                    if gps:
                                ...
                                for comp_info in components:
                                    comp_name = comp_info.get(
                                        "component_name"
                                    ) or comp_info.get("component_id", "Unknown")
                                    ...
                                    st.markdown(
                                        f"- **{comp_name}** (Component ID: `{comp_id}`, Score: {comp_score:.2f})"
                                    )
                                    ...
                                    if component_mapping and comp_id:
                                        ...
                                        if alternatives:
                                            with st.expander(
                                                f"Show alternative facilities for {comp_name}",
                                                expanded=False,
                                            ):
                                                st.markdown(
                                                    f"**Alternative facilities for {comp_name}:**"
                                                )
                                                for alt_tree in alternatives:
                                                    alt_facility = alt_tree.get(
                                                        "facility_name", "Unknown"
                                                    )
                                                    alt_score = alt_tree.get(
                                                        "confidence_score", 0
                                                    )
                                                    st.markdown(
                                                        f"- {alt_facility} (Score: {alt_score:.2f})"
                                                    )
                    ...
                    if production_sequence and all_trees_parsed:
                        ...
                        if stages:  # Only show if we have stages after filtering
                            ...
                            st.info(
                                "📋 **Component-Based Production**: Each component can be produced by the facilities listed. Facilities in the same stage can work in parallel."
                            )
                            st.caption(
                                f"Selected solution: {len(selected_facility_names) if selected_facility_names else 'all'} facilities across {len(stages)} production stage(s)"
                            )
                            ...
                            for stage in stages:
                                ...
                                for tree in trees_in_stage:
                                    ...
                                    comp_name = get_component_name_for_tree(
                                        tree, comp_name_map
                                    )
                                    ...
                                    location_info = tree.get("metadata", {}).get(
                                        "facility_location", {}
                                    )
                                    ...
                                    if location_info:
                                        ...
                                        if not city:
                                            city = location_info.get("address", {}).get(
                                                "city", ""
                                            )
                            ...
                            for comp_id, comp_info in sorted(
                                component_to_facilities.items(),
                                key=lambda x: (x[1]["stage"], x[1]["name"]),
                            ):
                                ...
                                with st.expander(
                                    f"🔧 **{comp_name}** (Stage {stage_num}) - {len(facilities)} facility/facilities available",
                                    expanded=False,
                                ):
                                    st.caption(
                                        f"Component ID: `{comp_id}` | Production Stage: {stage_num}"
                                    )
                                    ...
                                    for idx, facility in enumerate(facilities, 1):
                                        ...
                                        with col3:
                                            st.markdown(
                                                f"Score: {facility['confidence']:.2f}"
                                            )

                                    if len(facilities) > 1:
                                        st.caption(
                                            f"💡 Multiple facilities available - choose based on location, capacity, or lead time"
                                        )

                                    # Show stage dependency info
                                    if stage_num > 1:
                                        st.info(
                                            f"⏱️ Stage {stage_num} - Requires completion of earlier stages first"
                                        )
                    ...
                    if not deduplicated_facilities:
                        ...
                        for idx, facility_info in enumerate(facilities_in_solution, 1):
                            ...
                            trees = facility_info.get(
                                "trees", [facility_info.get("tree", {})]
                            )
                            confidence = facility_info.get("confidence", 0)

                            if len(trees) > 1:
                                st.markdown(
                                    f"**#{idx} {facility_name}** (handles {len(trees)} components, Score: {confidence:.2f})"
                                )
                                for tree_info in trees:
                                    if (
                                        isinstance(tree_info, dict)
                                        and "tree" in tree_info
                                    ):
                                        tree = tree_info["tree"]
                                    else:
                                        tree = (
                                    ...
                                    st.markdown(
                                        f"  - Component: **{comp_name}** (Score: {comp_confidence:.2f})"
                                    )
                            else:
                                tree = (
                                    trees[0].get("tree", trees[0])
                                ...
                                dep_indicator = get_facility_dependency_indicator(
                                    facility_info
                                )
                                ...
                                with st.expander(title, expanded=(idx == 1)):
                                    ...
                                    with col2:
                                        ...
                                        if not location_info or not location_info.get(
                                            ...
                                            facility = facility_by_name.get(
                                                facility_name
                                            )
                                            if facility:
                                                location_info = facility.get(
                                                    "location", {}
                                                )
                                        ...
                                        facility_id = tree.get(
                                            "okw_reference"
                                        ) or facility_info.get("tree", {}).get(
                                            "okw_reference"
                                        )
                                        if facility_id:
                                            st.markdown(
                    ...
                    if selection_criteria and total_solutions > 1:
                        ...
                        st.caption(
                            f"📊 Solution selected using: **{selector_display}** (from {total_solutions} total solutions)"
                        )
                    ...
                    sorted_solutions = sorted(
                        ...
                        key=lambda s: s.get(
                            ...
                            s.get(
                                "confidence",
                                s.get("tree", {}).get("confidence_score", 0),
                            ),
                        ),
                        reverse=True,
                    )
                    ...
                    for idx, sol in enumerate(sorted_solutions, 1):
                        tree = sol.get("tree", {})
                        facility_name = sol.get("facility_name") or tree.get(
                            "facility_name", "Unknown"
                        )
                        confidence = sol.get(
                            "confidence",
                            sol.get("score", tree.get("confidence_score", 0)),
                        )
                        match_type = sol.get(
                            "match_type", tree.get("match_type", "unknown")
                        )

                        # Get solution score (prefer score, then confidence, then tree confidence_score)
                        solution_score = sol.get(
                            "score",
                            sol.get("confidence", tree.get("confidence_score", 0)),
                        )

                        with st.expander(
                            f"#{idx} {facility_name} (Score: {solution_score:.2f})",
                            expanded=(idx == 1),
                        ):
                            col1, col2 = st.columns(2)
                            with col1:
                                ...
                                facility_id = sol.get("facility_id") or tree.get(
                                    "facility_id"
                                )
                                if facility_id:
                            ...
                            if materials:
                                ...
                                for mat in materials[:3]:  # Show first 3
                                    if isinstance(mat, str):
                                        ...
                                        name = mat.get(
                                            "name", mat.get("material_id", "Unknown")
                                        )
                                        qty = mat.get("quantity")
                                        unit = mat.get("unit", "")
                                        if qty:
            ...
            if display_facility_count > 0:
                ...
                st.markdown(
                    "Generate Request for Quotation documents for matched facilities"
                )
                ...
                if st.button(
                    "📄 Generate RFQs", type="primary", use_container_width=True
                ):
                    try:
                        with st.spinner("Generating RFQs... This may take a moment."):
                            ...
                            if (
                                not selected_design
                                and "selected_design_id" in st.session_state
                            ):
                                designs = get_okh_designs_for_selection()
                                selected_design = get_design_by_id(
                                    st.session_state["selected_design_id"], designs
                            ...
                            if not selected_design:
                                st.error(
                                    "⚠️ Design information not available. Please select a design and run matching again."
                                )
                            else:
                                ...
                                if design_data:
                                    ...
                                    if (
                                        "name" not in design_data
                                        and selected_design.get("name")
                                    ):
                                        design_data["name"] = selected_design.get(
                                            "name"
                                        )
                                    # Ensure other metadata fields are available
                                    if (
                                        "function" not in design_data
                                        and selected_design.get("function")
                                    ):
                                        design_data["function"] = selected_design.get(
                                            "function"
                                        )
                                    if (
                                        "description" not in design_data
                                        and selected_design.get("description")
                                    ):
                                        design_data["description"] = (
                                            selected_design.get("description")
                                        )
                                    if (
                                        "version" not in design_data
                                        and selected_design.get("version")
                                    ):
                                        design_data["version"] = selected_design.get(
                                            "version"
                                        )
                                else:
                                    # Fallback to metadata only
                                    design_data = {
                                        "name": selected_design.get(
                                            "name", "Unknown Design"
                                        ),
                                        "id": selected_design.get("id", "N/A"),
                                        "function": selected_design.get("function", ""),
                                        "description": selected_design.get(
                                            "description", ""
                                        ),
                ...
                if "rfqs" in st.session_state and st.session_state["rfqs"]:
                    ...
                    st.caption(
                        "💡 **Note**: These RFQs are for demonstration purposes. They show how RFQs could be generated from matching results."
                    )
                        ...
                        with st.expander(title, expanded=(idx == 1)):
                            ...
                            with col2:
                                if components:
                                    ...
                                    if len(components) > 3:
                                        st.caption(
                                            f"... and {len(components) - 3} more"
                                        )
        else:
            st.warning(
                f"Matching completed with status: {result.get('status', 'unknown')}"
            )
