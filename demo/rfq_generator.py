...
def extract_facility_info(
    tree_data: Dict[str, Any], facility_data: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
    ...
    info["location_string"] = (
        ", ".join(location_parts) if location_parts else "Location not specified"
    )
...
def extract_design_info(design_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    info["specifications"] = (
        " | ".join(spec_parts) if spec_parts else "See design documentation"
    )
...
def calculate_quantity_allocation(
    total_quantity: int, facility_count: int
) -> Dict[str, int]:
    """
...
def extract_manufacturing_requirements(tree_data: Dict[str, Any]) -> Dict[str, str]:
    ...
    process_list = (
        ", ".join(process_names) if process_names else "See design requirements"
    )
    ...
    material_list = (
        ", ".join(material_names) if material_names else "See design specifications"
    )

    return {"process_list": process_list, "material_list": material_list}
    ...
    if matching_mode == "nested":
        ...
        if not deduplicated_facilities:
            ...
            for facility_info in facilities_in_solution:
                ...
                component_id = facility_info.get("component_id") or tree.get(
                    "component_id"
                )
                component_name = (
                    facility_info.get("component_name")
                    or tree.get("component_name")
                    or design_info["name"]
                )
                ...
                allocated_quantity = allocation.get(
                    facility_index, quantity // facility_count
                )

                # Generate RFQ
                component_section = (
                    f"- Component: {component_name}" if component_id else None
                )
            ...
            for idx, facility in enumerate(deduplicated_facilities):
                ...
                for comp_info in components:
                    comp_name = comp_info.get("component_name") or comp_info.get(
                ...
                primary_component = (
                    component_names[0] if component_names else design_info["name"]
                )
