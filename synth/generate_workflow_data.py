...
def generate_workflow1_okh(okh_gen: OKHGenerator) -> OKHManifest:
    ...
    manifest.parts = [
        PartSpec(name="Hand Frame", tsdc=["3DP"], material="PLA"),
        PartSpec(name="Finger Joints", tsdc=["3DP"], material="TPU"),
        PartSpec(name="Assembly Components", tsdc=["Assembly"], material=None),
    ]
    ...
    manifest.manufacturing_specs = ManufacturingSpec(
        process_requirements=[
            ...
            ProcessRequirement(
                ...
                parameters={"support_removal": "required", "sanding": "optional"},
            ),
            ProcessRequirement(
                process_name="Assembly", parameters={"tools": ["screwdriver", "pliers"]}
            ),
        ],
        outer_dimensions={"length": 200, "width": 100, "height": 50},
    )
...
def generate_workflow1_okw_facilities(
    okw_gen: OKWGenerator, count: int = 5
    ...
    for i in range(2):
        facility = okw_gen.generate_specialized_facility(
            specialization="3DP", facility_index=i + 1
        )
    ...
    for i in range(2):
        facility = okw_gen.generate_specialized_facility(
            specialization="Assembly", facility_index=i + 1
        )
        facilities.append(facility)
    ...
    facility = okw_gen.generate_specialized_facility(
        specialization="3DP", facility_index=3
    )
    # Customize for post-processing
    facility.name = "Post-Processing Facility 1"
    facility.description = (
        "Specialized in post-processing and finishing of 3D printed parts"
    )
...
def generate_workflow2_okh(okh_gen: OKHGenerator) -> OKHManifest:
    ...
    manifest.function = (
        "Complete IoT sensor node with PCB, 3D printed enclosure, and final assembly"
    )
    ...
    manifest.parts.append(PartSpec(name="Sensor PCB", tsdc=["PCB"], material="FR4"))

    # Component 2: Enclosure (top-level part)
    manifest.parts.append(
        PartSpec(name="3D Printed Enclosure", tsdc=["3DP"], material="PLA")
    )

    # Component 3: Final Assembly (top-level part)
    manifest.parts.append(
        PartSpec(name="Final Assembly", tsdc=["Assembly"], material=None)
    )
    ...
    manifest.manufacturing_specs = ManufacturingSpec(
        ...
        outer_dimensions={"length": 80, "width": 60, "height": 25},
    )
...
def generate_workflow2_okw_facilities(
    ...
    pcb_facility = okw_gen.generate_specialized_facility(
        specialization="PCB", facility_index=1
    )
    ...
    printing_facility = okw_gen.generate_specialized_facility(
        specialization="3DP", facility_index=1
    )
    ...
    assembly_facility = okw_gen.generate_specialized_facility(
        specialization="Assembly", facility_index=1
    )
...
def main():
    ...
    if args.workflow in ["1", "both"]:
        ...
        okh1_path = (
            ...
            / "workflow1"
            / "okh"
            / f"{okh1.title.lower().replace(' ', '-')}-{okh1.version}-okh.json"
        )
        ...
        for i, facility in enumerate(okw1_facilities):
            facility_path = (
                ...
                / "workflow1"
                / "okw"
                / f"{facility.name.lower().replace(' ', '-')}-{i+1:03d}-okw.json"
            )
    ...
    if args.workflow in ["2", "both"]:
        ...
        okh2_path = (
            ...
            / "workflow2"
            / "okh"
            / f"{okh2.title.lower().replace(' ', '-')}-{okh2.version}-okh.json"
        )
